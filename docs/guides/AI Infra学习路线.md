...
- **ZeRO 拆解**：有人问你“ZeRO-2 和 ZeRO-3 到底差在哪”，你能讲清：ZeRO-2 只在 backward 时按需 AllReduce 梯度，参数每卡各存一份；ZeRO-3 连参数也切了，forward/backward 都要 AllGather 拿参数、用完即弃，通信量约多 50%（$3\Psi$ vs DDP 的 $2\Psi$，推导见 2.1 §5.2 的通信量表）但每卡显存降到 1/N
...
学习模块之外，建议按 **[Paper 精读](/AIInfraGuide/papers/)** 的路线并行阅读原文，给每个机制补上“一手来源”视角。精读按学习顺序组织，不是按发布时间平铺：

| 站 | 主题 | 读完你能… |
| --- | --- | --- |
| 第 0 站 | 直觉启动 | 明白投机解码在省什么、三笔账怎么算 |
| 第 1 站 | 推理引擎主干 | 讲清 KV 管理、batching、前缀缓存与 PD 解耦——推理岗 80% 系统面经题 |
| 第 2 站 | 投机解码谱系 | 对比自回归 / 并行 / 半自回归 drafter 路线 |
| 第 3 站 | 注意力与长上下文 | 解释在线 softmax、split-K 与稀疏注意力 |
| 第 4 站 | 分布式训练 | 训练方向：显存账本与并行策略 |
| 第 5 站 | 生产与前沿（并行阅读） | 生产架构视野与面试谈资 |

每篇标注阶段、站内顺序、预计时长与前置阅读；第 0–4 站按顺序读，第 5 站随时并行。精读与四大模块互补：模块负责建立体系，精读负责回到原文验证机制。
