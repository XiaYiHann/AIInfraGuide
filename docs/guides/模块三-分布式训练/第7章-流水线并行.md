...
- **跨机激活值传输**：Stage 间传输的数据量 = $b \times s \times h$（batch（批大小）× seq_len（序列长）× hidden（隐藏维）），需 IB（InfiniBand，跨机高速网络，单链约 200 Gb/s≈25 GB/s，远慢于机内 NVLink 的 900 GB/s，见 [5.7 多卡互联拓扑](/AIInfraGuide/prerequisites/模块一-前置知识/gpu/57-多卡互联拓扑#1-nvlink-是什么)——该节对比 NVLink/IB 带宽；本文只计 Stage 间激活量）带宽支撑
