...
- ❌ **单卡脚本直接加 `torchrun` 就以为在跑分布式**：模型和数据没做切分。为什么错：torchrun 只是启动了多进程，DDP 需要 `DistributedDataParallel` 包装 + `DistributedSampler` 切数据，否则每个进程都在重复训练同一份。正确做法：按本文的改造清单（init → wrap → sampler → barrier）逐步改。
- ❌ **`MASTER_ADDR`/`MASTER_PORT` 配错或漏配**：多机启动时报连接拒绝。为什么错：rank 0 是协调者，所有进程都要能连上它；端口被占或地址写错都会失败。正确做法：rank 0 写实际 IP，端口选不冲突的（如 29500 起），`--rdzv_endpoint` 与 `MASTER_ADDR` 二选一保持一致。
- ❌ **`--nproc_per_node` 与物理卡数不一致**：8 卡机器只起 4 个进程，或反过来。为什么错：进程数与卡数不匹配导致卡闲置或进程争抢。正确做法：`nproc_per_node` = 卡数，`nnodes` = 节点数，`node_rank` 从 0 编号。
- ❌ **不知道 `WORLD_SIZE` 等环境变量从哪来**：手动 export 一堆变量。为什么错：torchrun 会自动注入 `RANK`/`LOCAL_RANK`/`WORLD_SIZE` 等环境变量，手动设置反而覆盖出错。正确做法：用 `torchrun` 启动，进程内读环境变量，不要手写启动脚本拼变量。
