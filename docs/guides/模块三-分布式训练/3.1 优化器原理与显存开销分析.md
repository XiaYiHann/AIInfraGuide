...
- ❌ **混合精度下用 BF16 直接做优化器更新**：前向反向都用 BF16，优化器也省一半。为什么错：更新量太小（相对权重），BF16 精度不够，训练发散或收敛慢；优化器必须保留 FP32 主权重。正确做法：FP32 主权重 + BF16 计算副本（master weights）。
- ❌ **只算“权重 + 梯度”的显存**：忽略优化器状态。为什么错：AdamW + 混合精度下优化器状态占 12 字节/参数，是权重的 6 倍——7B 模型优化器状态约 84GB。正确做法：把优化器状态算进显存账本（本文给出完整分解）。
- ❌ **数据并行把 batch 放大 10 倍，学习率也直接乘 10**：线性缩放学习率。为什么错：大 batch 的梯度方差更小，线性缩放容易震荡甚至发散，需要 warmup 与 LARS/LAMB 类自适应。正确做法：按 LARS/LAMB 的规则缩放，或保持学习率用梯度累积验证。
- ❌ **把 SGD/Momentum/Adam 当成“同一种东西换参数”**：只记得 Adam“好用”。为什么错：每个优化器对应不同的状态存储（0/1/2 份/参数）与更新语义，选型直接决定显存与收敛行为。正确做法：能画出状态演化表（本文的检验清单第一项），再按场景选型。
...
- 能画出 SGD → Momentum → Adam 的状态演化表（每参数存储份数），并说出 ZeRO-1 切的是哪一份状态
- 能解释为什么大 Batch 训练要用 LARS/LAMB 而不是直接线性放大学习率
- 能用 `torch.cuda.memory_summary()` 或等价工具实测一个最小训练循环的峰值显存，与手算账本对比误差不超过 20%
