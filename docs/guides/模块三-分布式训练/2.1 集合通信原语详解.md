...
<img src="https://nanotron-ultrascale-playbook.static.hf.space/assets/images/a0_broadcast.png" alt="Broadcast 原语数据流示意" style="max-width: 100%; display: block; margin: 0 auto;" />
...
<img src="https://nanotron-ultrascale-playbook.static.hf.space/assets/images/a0_reduce_allreduce.png" alt="Reduce 与 AllReduce 原语数据流示意" style="max-width: 100%; display: block; margin: 0 auto;" />
...
<img src="https://nanotron-ultrascale-playbook.static.hf.space/assets/images/a0_scatter_reducescatter.png" alt="Scatter 与 ReduceScatter 原语数据流示意" style="max-width: 100%; display: block; margin: 0 auto;" />
...
<img src="https://nanotron-ultrascale-playbook.static.hf.space/assets/images/a0_gather_allgather.png" alt="AllGather 原语数据流示意" style="max-width: 100%; display: block; margin: 0 auto;" />
...
- ❌ **把 AllReduce 当成“一条原语调用”而不理解它内部是两阶段**：遇到通信调优直接搜“怎么加速 AllReduce”。为什么错：AllReduce = ReduceScatter + AllGather，不懂分解就不知道瓶颈在归约还是广播。正确做法：按本文的分解理解每个阶段的通信量（约 M + 约 M = 2M）。
- ❌ **阻塞式 Send/Recv 顺序写错导致死锁**：两个进程都“先 send 后 recv”。为什么错：底层缓冲区不够时双方互相等对方接收，永久卡死。正确做法：用异步 isend/irecv + wait()，或约定严格的收发顺序（低 rank 先收高 rank 先发）。
- ❌ **把通信量公式当常数背**：不区分数据量与通信量的差别。为什么错：AllReduce 通信量 ≈ 2M 与 Ring 的 2(N-1)/N×M 是针对不同问题（数据量 vs 每卡经手量）。正确做法：区分“总通信量”与“每卡链路经手量”，后者才是瓶颈（bus 视角）。
- ❌ **跨节点硬用依赖 NVLink 的算法**：在 InfiniBand 网络上默认 Tree/共享内存通道。为什么错：不同硬件拓扑的最优算法不同，NCCL 默认不一定最优。正确做法：用 nccl-tests + NCCL_DEBUG 实测各算法，再固定最优配置（见 6.5）。
...
- 能默写 Ring AllReduce 的通信量公式 2(N-1)/N × M 并给出 N=4 的数值算例
