...
- **有效 batch size 变大了**：数据并行的有效 batch size = 单卡 local batch × $N$。这也是为什么扩大卡数时通常要相应调整学习率（详见 [4.2 PyTorch 数据并行从原理到实战 §6.4 学习率缩放](/AIInfraGuide/distributed/模块三-分布式训练/42-pytorch-数据并行从原理到实战#64-学习率缩放)——该节推导 $lr$ 随 $N$ 线性缩放及 Warmup；本文只证有效 batch 的数学等价，缩放实现见该节）。
...
> 💡 **回顾：AllReduce 是什么**
> 各卡把各自算出的部分和相加，最后每张卡都拿到同一份完整结果；Ring 实现通信量≈$2(N-1)/N\cdot\Psi\approx2\Psi$，与卡数无关。详见 [第2章 集合通信原语 §3 集合通信原语详解](/AIInfraGuide/distributed/模块三-分布式训练/第2章-集合通信原语#3-集合通信原语详解)——该章给出 AllReduce/AllGather/ReduceScatter 的语义与 Ring 推导；本节在此基础上每步 1 次同步梯度。
...
计算与通信重叠让 DDP 在中等规模下几乎能线性加速，但这个红利在**超大规模会失效**（如512卡时Ring延迟$2(N-1)\alpha$≈5ms，2$\Psi$=28GB BF16经400Gbps IB需560ms盖不住10ms计算，HBM 3TB/s读权重仅4.7ms→通信暴露，矛盾→极限→失效→打破：任务欲线性加速→受$2\Psi$带宽极限→超大N时失效→转TP/PP/ZeRO打破）。随着 GPU 越来越多（数百甚至上千块），协调开销和网络需求显著增长，通信逐渐盖不住计算，每多加一块卡效率反而更低。下面是一张示意图（非本站实测，数值仅作趋势示意，待核实）：超过某个上限后吞吐量开始明显下降，而单卡显存占用保持恒定、不受 DP rank 增加的影响（这也印证了前面那句话——加卡只摊薄计算、不摊薄显存）。
...
📌 **关键点**：DDP 增加卡数只能**摊薄计算时间**（更快），不能**摊薄单卡显存**（更省，Shape守恒：梯度$[\Psi]$每卡完整→FSDP切为$[\Psi/N]$）。要突破单卡显存墙，就要问：这份 $16\Psi$ 的冗余，能不能也切开分给各卡？——这正是 FSDP 的出发点。
> 💡 Roofline一瞥：7B BF16 14GB权重 HBM读4.7ms vs NVLink 900GB/s AllReduce 28GB需31ms（Byte=2×Ψ），FSDP 3Ψ=42GB需47ms→显存14GB vs 112GB（降低8×）换通信1.5×（HBM/SRAM/NVLink/Byte四账本）
> 图中FSDP流程第2箭头“AllGather拼完整参数”对应正文“计算某一层时通过AllGather临时收集”一步
...
- ✅ **单个层都装不下**（如超大 Embedding 或 FFN） → 数据并行这条维度到头了，要叠加张量并行（TP）、流水线并行（PP），走向 3D 并行（详见 [第6章 张量并行与序列并行](/AIInfraGuide/distributed/模块三-分布式训练/第6章-张量并行与序列并行)——该章讲列/行切与每层 AllReduce；[第7章 流水线并行](/AIInfraGuide/distributed/模块三-分布式训练/第7章-流水线并行)——该章讲 1F1B 调度与 bubble；[第11章 3D并行与混合并行策略](/AIInfraGuide/distributed/模块三-分布式训练/第11章-3d并行与混合并行策略)——该章讲 TP×PP×DP 拓扑；本文只管数据并行维度）。
...
- ❌ **把 DP（单进程多线程）当生产方案**：`DataParallel` 一个进程里塞多个 GPU。为什么错：GIL 争抢 + 每步复制模型，通信与同步开销大，速度远不如 DDP。正确做法：生产用 DDP（每卡一个进程，无 GIL）。
- ❌ **DDP 里每卡喂相同的数据**：忘记 `DistributedSampler`。为什么错：每卡梯度相同，AllReduce 后等于单卡训练，浪费多卡。正确做法：用 `DistributedSampler` 切分数据，并注意 `drop_last` 与 epoch 后 `set_epoch`。
- ❌ **只看 DDP 的通信量 $2\Psi$ 就选型**：不比较 FSDP 的显存账本。为什么错：DDP 省通信但每卡显存 $16\Psi$（7B 装不下），FSDP 省显存但通信 $3\Psi$；没有“哪个更好”，只有“瓶颈是什么”。正确做法：单卡装得下用 DDP，装不下用 FSDP/ZeRO（本文的决策树）。
- ❌ **梯度累积与 DDP 混用时不注意同步次数**：accumulation step 里每个 micro-batch 都 AllReduce。为什么错：通信次数翻倍，收益被吃掉；正确做法：只在累积完成后同步一次梯度（PyTorch 2.x 的 `no_sync` 或累积结构）。
