...
- **计算过程**：$Y_i = X \cdot A_i$（Shape守恒：$[B,S,H]\times[H,H/N]\rightarrow[B,S,H/N]$，每卡用完整输入 $X$ 乘列分片）
...
- **计算过程**：$Y_i = X_i \cdot A_i$（Shape守恒：$[B,S,H/N]\times[H/N,H]\rightarrow[B,S,H/N]$部分和，AllReduce后得$[B,S,H]$）
...
- 💡 Roofline一瞥：AllReduce 2×$2bsh$ Byte vs SP拆分后通信量不变（$2bsh$ Byte, HBM 3TB/s读 $bsh$仅0.01ms，NVLink 900GB/s传16MB需0.018ms）激活从$bsh$→$bsh/N$（如$b$=1,$s$=4096,$h$=4096,N=8→32MB→4MB/卡）

> 状态机（TP↔SP衔接，图中箭头对应正文“进入/退出非TP区域”2步）：
> ```mermaid
> stateDiagram-v2
>     [*] --> TP_Compute: 完整输入[B,S,H]
>     TP_Compute --> ReduceScatter: 进入SP区域
>     ReduceScatter --> SP_Compute: 切分后[B,S/N,H]
>     SP_Compute --> AllGather: 回到TP区域
>     AllGather --> TP_Compute
>     TP_Compute --> [*]
> ```
