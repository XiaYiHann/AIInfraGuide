...
- 💡 Roofline一瞥：BF16 14GB权重 HBM 3TB/s读需4.7ms，FP32 28GB需9.3ms；NVLink 900GB/s同步14GB需15.5ms Byte瓶颈，Tensor Core BF16算力312TFLOPS vs FP32 66TFLOPS→算力翻4.7×（HBM/SRAM/NVLink/Byte四账本）
...
- **Effective Batch Size** = 单卡 batch × world_size × accumulation_steps  Shape守恒：梯度$[\Psi]$不变，显存峰值保持$[B_{micro},S,H]$，K步累积等效$[B_{eff},S,H]$（如$B_{micro}$=1,world=8,K=4→$B_{eff}$=32不增显存）
...
- **Full Checkpointing**：只保存每个 Transformer Block 的输入，反向时重算整个 Block
  - 显存节省：激活显存从 $O(L)$ 降至 $O(\sqrt{L})$（每 $\sqrt{L}$ 层存一个 checkpoint，如L=32→存6份从32$bsh$→6$bsh$降低5.3×）
  - 状态机（Checkpoint重算）：
    ```mermaid
    stateDiagram-v2
        [*] --> Forward: 保存[√L]检查点
        Forward --> Drop: 丢弃中间激活
        Drop --> Recompute: 反向需时重算[B,S,H]
        Recompute --> Backward: 用重算激活反向
        Backward --> [*]
    ```
    图中Drop→Recompute箭头对应正文“反向时重新前向计算”一步
  - 计算代价：约增加 33% 的前向计算量
