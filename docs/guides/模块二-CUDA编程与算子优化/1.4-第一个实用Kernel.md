...
stateDiagram-v2
    [*] --> 已分配: cudaMalloc [N]×4B
    已分配 --> 已上传: cudaMemcpy H2D (PCIe 32GB/s)
    已上传 --> 已启动: kernel<<<grid,block>>> [gridDim×blockDim→N]
    已启动 --> 已完成: SM 执行 idx<N?C[idx]=A+B
    已完成 --> 已拷回: cudaMemcpy D2H
    已拷回 --> 已释放: cudaFree/free
    已释放 --> [*]
```

> 图中“已上传→已启动→已完成→已拷回”四箭头对应正文五步流程的第 2–4 步与 CUDA Event 计时区间；`gridDim=(N+255)/256` 的取整对应边界检查 `if(idx<N)`。
...
> Shape 守恒：$A,B,C\!:\![N]$（$N=1\ll20$ 时 $12$ MB），线程 $idx=blockIdx.x\times blockDim.x+threadIdx.x\!:\![gridDim,blockDim]\to[N]$；Grid-stride 变体 $stride=blockDim.x\times gridDim.x$ 后 $i=idx + k\cdot stride$ 仍覆盖 $[N]$，写出 $C\!:\![N]$ 形状不变——否则丢尾或越界。
...
> **矛盾前置（任务→极限→失效→打破）**：任务——$N=2^{24}$ 向量加法每元素 1 FLOP 对 12 Byte（$AI=0.083$）；极限——RTX 4090 HBM $1008$ GB/s 峰值、PCIe $32$ GB/s、Kernel $AI$远低于平衡点；失效——朴素“CPU 循环 + 每次全量 H2D/D2H”中 PCIe 占 $≈96\%$（$8$ ms 中传输 $8$ ms），Kernel 仅 $0.3$ ms，再多算力也掩不住搬运；打破——数据驻留 GPU + Grid-stride 合并访存 + 多 Kernel 串联（后续 GEMM/Reduce 同理），把 HBM 往返从每步 3 次压到常驻，真实瓶颈转回 HBM 带宽与合并度。
