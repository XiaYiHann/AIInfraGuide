...
- ❌ **用 `data[tid * stride]` 的跨步访问“假装”并行**：stride > 1 时，一个 Warp 的 32 个线程访问的地址彼此相隔 stride 个元素。为什么错：每个 32 字节内存事务只用到其中一小段，有效带宽骤降（跨步越大越严重）。正确做法：让连续线程访问连续地址，需要跨步时先 coalesced 读入 Shared Memory 再转置。
- ❌ **不检查指针与偏移的对齐就上向量化**：`float4` 要求 16 字节对齐，偏移计算错一位就退化为慢路径。为什么错：对齐错误的向量访问要么报错、要么被编译器拆成多次标量加载，收益全丢。正确做法：分配时保证基地址对齐，偏移量用元素数 × 元素大小的整数倍。
- ❌ **把所有 Kernel 都当成 Compute-Bound 来优化**：在 Memory-Bound 的 Kernel（Elementwise、Reduce、转置）上死磕指令数。为什么错：这类 Kernel 的时间由访存决定，少几条指令毫无影响。正确做法：先用 Roofline/理论带宽算清算力强度，Memory-Bound 就优化访存模式。
- ❌ **忽略 L2 缓存与数据的局部性**：反复从 HBM 读同一块数据。为什么错：L2 命中比 HBM 快一个量级，白读 HBM 浪费带宽。正确做法：分块（Tiling）让数据在寄存器/Shared Memory/L2 中复用。
