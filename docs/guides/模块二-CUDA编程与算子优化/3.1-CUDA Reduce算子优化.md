...
<img src="/AIInfraGuide/images/reduce-cuda-kernel-01.png" alt="并行规约的树形两两相加示意" style="max-width: 90%; display: block; margin: 0 auto;" />
...
<img src="/AIInfraGuide/images/reduce-cuda-kernel-02.png" alt="树形规约每轮活跃线程范围示意" style="max-width: 90%; display: block; margin: 0 auto;" />
...
- ❌ **照抄朴素树形规约不优化**：`if (tid % (2 * step) == 0)` 的写法。为什么错：每一步都产生 Warp Divergence，且随 step 增大利用率指数下降（本文 V0 实测带宽利用率仅约 15%）。正确做法：先改成 strided index（V1），再反转步长方向（V2）一次解决 Divergence 与 Bank Conflict。
- ❌ **在 Warp 内规约时滥用 `__syncthreads()`**：最后 32 个线程的规约还在循环里同步。为什么错：Warp 内天然同步，多余的屏障白花周期。正确做法：最后一个 Warp 展开循环（V4 的 warpReduce）。
- ❌ **忽略 Bank Conflict 只看“共享内存比全局快”**：以为数据进了 Shared Memory 就万事大吉。为什么错：步长 2 的访问就是 2-way Bank Conflict，串行化后吞吐减半。正确做法：按本文 V2 的步长反转或 padding 消除冲突。
- ❌ **多 Block 结果不做二次规约**：每个 Block 写一个部分和就收工。为什么错：`output[blockIdx.x]` 里是几万个部分和，不是最终结果。正确做法：再启动一次 Reduce（或 CPU 上求和）把部分和规约成最终结果。
