...
- [4. Thread Block 级 Tiling 优化](#4-thread-block-级-tiling-优化)
- [5. Thread 级 Tiling 优化：寄存器级数据复用](#5-thread-级-tiling-优化寄存器级数据复用)
...
远低于平衡点（17 FLOPs/Byte），严重受带宽限制。工程经验值：通常只有理论峰值的 6%~11%（A100 量级，本站未在文中声明测试环境，待核实）。
...
- 工程经验值：通常带来 15%~30% 的性能提升（A100 量级，待核实）
...
<img src="/AIInfraGuide/images/gemm-7.png" alt="方案三：8 x 8 计算区域拆分为 4 x 4 子块" style="max-width: 80%; display: block; margin: 0 auto;" />
...
<img src="/AIInfraGuide/images/gemm-8.png" alt="方案四：Z-order 线程映射" style="max-width: 80%; display: block; margin: 0 auto;" />
...
💡 **口径说明**：上表为不同优化阶段相对 cuBLAS 的性能区间，系工程经验估算（A100 量级、FP32 大矩阵场景），本文未给出逐项实测环境，数字待核实；各优化手段的相对收益排序才是本表要传递的信息。
