...
> 💡 **Roofline 一瞥（A100: 312 TFLOPS FP16 / 2 TB/s HBM → 平衡点≈156 FLOP/Byte；SRAM ~19 TB/s，NVLink 600 GB/s）**：同步/原子类 Kernel 算术强度 $AI\ll1$，瓶颈在 HBM/L2 往返与等待而非算力；`__syncthreads` 仅 5–20 cycles（SRAM 驻 SM），全局 `atomicAdd` 需 400–600 cycles 走 L2/HBM（约 0.3μs @1.5 GHz），跨 Block 栅栏需一次 HBM 可见性往返。

| 账本 | 指标 | 量级（A100 80GB） |
|------|------|-------------------|
| HBM | 全局 `atomicAdd` 延迟 / 带宽 | 400–600 cycles · 2 TB/s |
| SRAM | `__syncthreads` / `__syncwarp` / Shfl | 5–20 cycles · ~19 TB/s（SM 内） |
| 同步等待 | Warp 队列串行化（32 线程争 1 地址） | 最差 32× 串行，仅 3% 有效带宽 |
...
**全局内存上的原子操作**延迟约 400\~600 cycles（需要一路走到 L2 或 DRAM 完成操作；数量级为工程经验值，待核实）。**共享内存上的原子操作**延迟约 20~100 cycles（在 SM 内部完成；同上，待核实）。
...
stateDiagram-v2
    [*] --> 读旧值: 载入 *addr
    读旧值 --> CAS尝试: atomicCAS(addr, assumed, new)
    CAS尝试 --> 成功: 旧值==assumed
    CAS尝试 --> 重试: 旧值!=assumed（被他人抢改）
    重试 --> 读旧值: 更新 assumed=old 再试
    成功 --> [*]: 返回 old
```

> 图中“CAS尝试→重试”环对应正文 `do{assumed=old; old=atomicCAS(...);}while(assumed!=old)` 的自旋语义；高竞争时该环平均重试次数随并发度线性增长。

---

## 6. 原子操作的性能优化

> **矛盾前置（任务→极限→失效→打破）**：任务——数千线程需对同一计数器做 `atomicAdd` 归约；极限——HBM 原子在内存控制器处串行化，单地址每周期仅 1 次 RMW；失效——朴素“全员直写同一地址”在 Warp 内即 32× 串行、跨 Block 时延迟 400–600 cycles 叠加，有效带宽跌至~3%；打破——分层归约（Warp Shfl→Block 共享→Grid 单原子，见下）把 N 路竞争压到 $gridDim.x$ 路，冲突与 HBM 往返同步下降。
