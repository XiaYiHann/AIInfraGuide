...
- ❌ **在 V2 里沿用 V1 的“沿 K/V 分 Warp”切分**：4 个 Warp 各自负责一段 K/V。为什么错：每个 Warp 都要维护一份自己的局部 m/l/O，跨 Warp 归约次数随 K/V 块数线性增长，同步开销大（V1 方案）。正确做法：沿 Q 分 Warp——每个 Warp 负责不同的 Q 行块，互不依赖，零跨 Warp 归约。
- ❌ **把 V2 的加速全部归功于“循环交换”**：只记住“交换了循环顺序”。为什么错：V2 的收益来自三件事——Q 在外循环消除 m/l/O 的 HBM 往返、沿 Q 分 Warp 减少同步、以及不做昂贵的 rescale。正确做法：三个改进分开讲，各自对应一个性能瓶颈。
- ❌ **保存 m 和 l 两个张量用于反向**：以为前向必须存 $m_i$ 与 $l_i$。为什么错：V2 只存行级 logsumexp $L_i = m_i + \log(l_i)$，省一份显存。正确做法：反向时从 L 恢复缩放因子，而不是重新存两个张量。
- ❌ **假设 FA 对任意硬件参数都最优**：在块大小、Warp 数上不做调优直接上线。为什么错：最优分块依赖 SRAM 大小与 Tensor Core 形状，不同 GPU 差异很大。正确做法：用 Nsight Compute 对比不同块大小的实际吞吐。
...
- 能说出 V2 相对 V1 交换循环顺序后，m/l/O 为什么能全程留在寄存器（减少一次 HBM 读写往返）
