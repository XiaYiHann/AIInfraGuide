...
> **矛盾前置（任务→极限→失效→打破）**：任务——每 SM 数千线程需 $312$ TFLOPS 算力喂饱；极限——HBM 仅 $2$ TB/s、SRAM $19$ TB/s、NVLink $600$ GB/s，片外带宽为瓶颈；失效——若全部走 HBM 随机访存，$AI=0.25$ FLOP/Byte 时仅 $500$ GFLOPS（$0.2\%$ 峰值），延迟 $400$ cycles；打破——分层驻留（寄存器 $1$ cy→SMEM $20$ cy→HBM $400$ cy）+ 合并访存 + Tiling“搬一次用多次”，把 HBM 流量压到 SRAM 可覆盖，见下 §9。
...
<img src="/AIInfraGuide/images/cuda内存模型.png" alt="CUDA 内存层次架构图" style="max-width: 70%; display: block; margin: 0 auto;" />
...
<img src="/AIInfraGuide/images/合并访存与非合并访存对比.png" alt="合并访存与非合并访存对比示意图" style="max-width: 70%; display: block; margin: 0 auto;" />
...
```mermaid
stateDiagram-v2
    [*] --> 驻留主机: cudaMallocManaged [N]×4B 初始在 Host
    驻留主机 --> 缺页GPU: Kernel 访问触发缺页中断
    缺页GPU --> 驻留显存: 驱动迁移页到 HBM (PCIe/NVLink)
    驻留显存 --> 执行: SM 计算完成
    执行 --> 缺页主机: CPU 访问触发回迁
    缺页主机 --> 驻留主机: 迁回 Host
    驻留显存 --> 预取优化: cudaMemPrefetchAsync 显式迁移
    预取优化 --> 驻留显存
```

> 图中“缺页GPU→驻留显存”箭头对应正文“GPU Kernel 访问时触发缺页中断，驱动将页面迁移到显存”；“预取优化”环对应 §8.3 预取避免按需迁移的 $2$ 次缺页延迟（每次约 $10$–$20$ μs）。
...
```cpp
__global__ void matmulTiled(float* A, float* B, float* C, // Shape: A[M,K]×B[K,N]→C[M,N]
                            int M, int N, int K) { // Tile: As[TILE,TILE], Bs[TILE,TILE]；行列映射 row=By·T+ty [M] col=Bx·T+tx [N] → C[row,N+col] [M,N]
...
- ❌ **只盯着全局内存优化，忽略 Shared Memory**：新手常把时间全花在 Global Memory 的合并访问上，却对 Shared Memory 的 Bank Conflict 视而不见。为什么错：Shared Memory 的带宽比全局内存高一个数量级，但冲突会让访问串行化，把优势吃光。正确做法：先确认瓶颈在哪一级内存（用 Nsight Compute 看 stall 原因），再针对性优化。
- ❌ **AoS（数组结构体）布局直接搬进 Kernel**：`struct Particle { float x, y, z; }` 的数组让相邻线程访问 `x, y, z, x, y, z...`，每个 12 字节里只有 4 字节有用。为什么错：合并访存要求相邻线程访问相邻地址，AoS 把有效数据隔开了。正确做法：改成 SoA（结构体数组），`x[]`、`y[]`、`z[]` 分开存放。
- ❌ **假设所有内存访问都自动合并**：跨步访问（stride > 1）会让一个 Warp 的 32 次访问摊到多个事务上。为什么错：事务数翻倍 = 有效带宽减半。正确做法：让 `tid` 连续对应相邻地址（`data[tid]` 而不是 `data[tid * 2]`）。
- ❌ **忽略 16 字节对齐**：用 `float4` 或向量化加载前不检查指针对齐。为什么错：未对齐的向量访问在部分架构上直接报错或退化成多次标量访问。正确做法：分配时用 `cudaMalloc`（天然 256B 对齐）或显式对齐分配器。
