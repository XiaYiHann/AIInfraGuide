...
---
标准 Attention 对长度为 $L$ 的序列要处理 $L\times L$ 对 token（词元，约 0.75 个汉字/词元）关系：每个新词元都要和它之前的所有词元两两配对打分。FlashAttention 通过重排计算让这张配对表不落到 HBM（High Bandwidth Memory，GPU 高带宽主显存），但它仍然计算全部配对；DeltaNet 则走另一条路——把“每个词元都记住”改成“边看边往一张固定大小的记忆表里写”。这类做法统称**线性注意力**（linear attention：通过重排乘法顺序、不构造 $L\times L$ 配对矩阵，把对序列长度的主项复杂度从 $O(L^2)$ 压到 $O(L)$ 的注意力变体），DeltaNet 再把写入规则升级成按预测误差修正旧记忆的 **Delta Rule**（先读出旧值，再按“目标值减旧预测”的差值改写）。

在单头示意下，它把随 $L$ 增长的 KV 记忆（即 KV Cache，历史 K/V 缓存，大小 $M=2N_{\text{layer}}N_{\text{head}}Ld_{\text{head}}b$，每层每头各存一份 K、V，见 [§1 KV Cache 与长序列的墙](#1-为什么需要这条路线kv-cache-与长序列的墙)）换成 $d\times d$ 状态，并用 chunk-wise（分块并行）算法把原本逐 token 的递归训练改成约 $L/C$ 个块级递推。**代价也很明确：状态容量有限，且“免 KV Cache”来自更换序列混合机制，不是对原 Attention 的无损 Kernel 优化。**
<!-- more -->
## 📑 目录
- [🗺️ 原文阅读地图](#-原文阅读地图)
...
| Delta update 与 key-value retrieval | **精讲** | §3.3，回答“为什么朴素线性注意力只能叠加、无法覆盖旧关联” | 主论文 §2.2 |
...
你可以把一个 token 的三种向量理解成三个角色：Query（$Q$）是“我想找什么”，Key（$K$）是“我有哪些可匹配的线索”，Value（$V$）是“匹配后真正要取出的内容”。Q、K、V 角色与标准 Attention 的完整推导，在模块一的 [3.3 Self-Attention 机制深入理解](/AIInfraGuide/prerequisites/模块一-前置知识/transformer/33-self-attention机制深入理解)（从零讲清 QKV、注意力分数与 KV Cache 的来龙去脉）已经讲过；本文只复用它的符号与形状，机制部分不再重讲。
...
$$
这个式子做三件事。第一，$QK^{\mathsf T}$ 计算每个 Query 和所有历史 Key 的匹配分数，结果形状是 $L\times L$。第二，Softmax（先把每个分数取指数，再除以这一行指数之和，得到总和为 1 的权重）把每一行分数变成权重。第三，用这些权重对 $V$ 加权求和，得到 $L\times d$ 的输出。
...
FlashAttention 解决的是“同一条 Attention 公式怎样少搬数据”。它把中间结果留在片上（SRAM，GPU 芯片内的高速缓存，容量远小于 HBM 但速度快得多），必要时用额外计算换掉显存存储，但没有把“当前 Query 要和所有历史 Key 比对”这个语义改掉（这条路线的完整展开见 [6.1 FlashAttention V1 详解](/AIInfraGuide/cuda/模块二-cuda编程与算子优化/61-flashattention-v1详解)——它讲同一套公式如何通过分块与重算避免写出 $L\times L$ 矩阵）。DeltaNet 解决的是“能不能换一种状态更新规则，从架构上不再逐个保存历史 K、V”。
...
只算一个 batch（一次同时送入模型的一组样本）、一个模型的 KV 张量，显存字节数可写成：
...
线性注意力（linear attention，名字里的“线性”指对序列长度 $L$ 的主项复杂度从 $O(L^2)$ 降到 $O(L)$，对比见下方两行）的想法不是先优化 KV Cache 的分页，而是重新安排矩阵乘法。先暂时忽略 Softmax，标准形式的核心是：
...
> Shape 守恒：$Q\!:\![L,d], K\!:\![L,d], V\!:\![L,d]$ 左式 $QK^{\mathsf T}\!:\![L,d]\times[d,L]\to[L,L]$ 再 $\times V\!:\![L,L]\times[L,d]\to[L,d]$；右式 $K^{\mathsf T}V\!:\![d,L]\times[L,d]\to[d,d]$ 再 $Q\times\!:\![L,d]\times[d,d]\to[L,d]$——输出 $[L,d]$ 一致，中间量从 $L^2$ 压到 $d^2$。
...
第二，朴素写入只有加法。每来一个键值对，就把一个外积（outer product：列向量乘行向量，得到一个 $d\times d$ 矩阵，§2.1 有最小算例）加到状态上；如果新 token 想复用某个旧 Key，系统没有明确的“先找到旧值，再把它擦掉”的动作。长序列里，这会变成容量和检索问题。
...
用技术语言重述，单头、identity feature map（恒等特征映射：$\phi(x)=x$，不改变输入）的线性注意力递归为：
...
```mermaid
stateDiagram-v2
    [*] --> 输入: x_t → k_t,v_t,q_t [d]
    输入 --> 读旧: S_{t-1}[d,d] × k_t[d] → v_old[d]
    读旧 --> 算误差: β_t∈(0,1), v_new=βv+(1-β)v_old
    算误差 --> 写状态: S_t=S_{t-1}-v_old·k^T+v_new·k^T [d,d]
    写状态 --> 读出: o_t=S_t·q_t [d]
    读出 --> 传递: S_t → 下一 token
    传递 --> [*]
```

> 图中“读旧→算误差→写状态”三步对应 §3.3 `v_old=S_{t-1}k_t→v_new→S_t=S_{t-1}-v_oldk^T+v_newk^T`；“传递”箭头对应 §3.5 块间仅传 $S_{[t]}$ 的语义，块内 $C\times C$ 三角依赖留在 SRAM 不落 HBM。

> 💡 **三账本一瞥（$L=4096,d=128,FP16$）**：KV Cache $2Ld≈1$M 元素/头→$2$ MB；DeltaNet 状态 $d^2=16$K 元素→$32$ KB，HBM 常驻量降 $64×$；但状态读写 $O(d^2)$/token（约 $32$K FLOP），在 HBM $2$ TB/s、SRAM $19$ TB/s、NVLink $600$ GB/s 下，瓶颈从 HBM 容量转向 SRAM 带宽与状态依赖的三角求解。
## 3. 方法拆解：从加法记忆到 DeltaNet 分块并行
### 3.1 朴素线性注意力：结合律把 L×L 矩阵变成 d×d 状态
#### 为什么要引入 kernel 技巧

标准 Softmax 的相似度是 $\exp(k^{\mathsf T}q)$，它通常不能直接拆成“只先汇总 K 和 V”的形式。线性注意力用一个 feature map（特征映射：把每个向量变换成另一个向量的小函数）$\phi$ 把相似度近似写成：
...
$$
每一项的含义是：$S_t$ 汇总 Value 和 Key 的外积，$z_t$ 汇总 Key 特征，Query 同时访问这两个状态。DeltaNet 主论文为了突出更新规则，主要使用 identity 形式的 $S_t$ 与 $o_t=S_tq_t$；在实际 DeltaNet layer 中，论文又对 Q、K 使用 SiLU（一种激活函数，见 §3.7 展开）和 L2 归一化（把向量除以自身长度），后文单独说明。
...
|---|---:|---:|---|---|
| Recurrent（逐 token 递推） | $O(Ld^2)$ | $O(L)$ | 状态小，但逐 token，难充分使用 Tensor Core（加速矩阵乘法的专用硬件单元） | 论文 §2.1 |
| Fully parallel | $O(L^2d+Ld^2)$ | $O(1)$ | 矩阵乘丰富，但临时关系矩阵大 | 论文 §2.1、§3.2 |
| Chunk-wise | $O(LCd+Ld^2)$ | $O(L/C)$ 块级递推 | 块内矩阵乘，块间传状态 | 论文 §3.2 |
📌 **关键点**：chunk-wise 不是凭空创造第四种数学结果，而是在“递归省 FLOPs（浮点运算次数，计算量的单位）”和“全并行吃 GPU”之间插入一个可以调节的旋钮 $C$。
...
\end{aligned}
$$
如果不同 Key 互相正交（方向互相垂直），串扰项为 0，检索才干净。可是在 $d$ 维空间里，最多只能放下 $d$ 个彼此正交的方向。FWP 论文把这称为有限容量的关联记忆，DeltaNet 主论文则直接指出：当 $L>d$ 时，Key collision 会变得困难。
**二维算例**：取
...
$$
用学习率 $\beta_t$ 做一步 SGD（随机梯度下降：每看一个样本就沿梯度反方向更新一步）：
...
$$
普通线性注意力只有“加一个外积”，不同时间步的加法可以合并（就像算前缀和——把前 $t$ 项逐项加起来，顺序无所谓，所以能一次扫完）；DeltaNet 每一步还要右乘一个不同的 $M_t$。矩阵乘法一般不可交换，因此不能把所有 $M_t$ 随意排序后一次算完。
...
- $k_t$ 方向的特征值（矩阵沿这个方向放大的倍数）是 $1-\beta_t$。
...
$$
这就是“定向遗忘”：只擦第一方向，不把整块状态清空。主论文把它称为 generalized Householder transformation（Householder 反射是线性代数里“沿一个方向做反射/投影”的矩阵操作，这里将其推广成“只在一个方向上动手”的擦除转移）的结构。

#### 把完整递推走一遍（2×2 矩阵）

仍用 $k=[1,0]^{\mathsf T}$、$\beta=1$（$M=\operatorname{diag}(0,1)$），这次把“减旧、加新”在矩阵层面走完整。设 $S_0=0$。第一步写入 $v_1=[3,5]^{\mathsf T}$：

$$
S_1=\underbrace{S_0M}_{0}+\underbrace{\beta_1 v_1k^{\mathsf T}}_{\begin{bmatrix}3\\5\end{bmatrix}\begin{bmatrix}1&0\end{bmatrix}=\begin{bmatrix}3&0\\5&0\end{bmatrix}}=\begin{bmatrix}3&0\\5&0\end{bmatrix}。
$$
用 $q=[1,0]^{\mathsf T}$ 读取得到 $S_1q=[3,5]^{\mathsf T}=v_1$——写什么就能读回什么。第二步用同一个 Key 写入 $v_2=[8,2]^{\mathsf T}$，先读旧预测 $S_1k=[3,5]^{\mathsf T}$（即 §3.3 的 $v_t^{\text{old}}$），再替换：

$$
S_2=\underbrace{\begin{bmatrix}0&0\\0&5\end{bmatrix}}_{\text{擦掉沿 }k\text{ 的第一列，保留正交的第二列}}+\underbrace{\begin{bmatrix}8&0\\2&0\end{bmatrix}}_{\text{写入新值 }v_2k^{\mathsf T}}=\begin{bmatrix}8&0\\2&5\end{bmatrix}。
$$
再读一次 $S_2q=[8,2]^{\mathsf T}=v_2$：沿 $k$ 方向的旧值 $[3,5]^{\mathsf T}$ 被整个换成 $[8,2]^{\mathsf T}$，这就是“先擦旧外积、再写新外积”在矩阵上的样子；而 $[0,5]^{\mathsf T}$ 位于与 $k$ 正交的方向，没被动过。
#### WY 表示把状态改写成一串秩一项

WY 是数值线性代数里“一连串 Householder 反射矩阵相乘”的紧凑表示——它们的乘积可以打包成 $I+WY^{\mathsf T}$ 这样的两块小矩阵，不必每步展开成完整矩阵；论文把同一思想用在这条递推上。如果每一步都把完整 $S_t$ 写到显存里，状态形状是 $d\times d$，序列每一步都会产生很重的 I/O。论文观察到，状态可以改写成：

$$
\underbrace{S_t=\sum_{i=1}^{t}u_ik_i^{\mathsf T}}_{\text{只保留秩一项的和}}。
$$
关键是 $u_t$ 不是原始 $v_t$，而是已经考虑旧 Key 之间相互作用、把干扰扣掉后的修正写入值（论文术语 pseudo value）：

$$
\underbrace{u_t=\beta_t\left(v_t-\sum_{i=1}^{t-1}u_i(k_i^{\mathsf T}k_t)\right)}_{\text{用 Key 的点积修正当前写入}}。
$$
每个 $u_t$ 是 $d$ 维向量，计算它时可以不 materialize（把中间量实体化写进显存）过去每一步的 $d\times d$ 隐状态。这里的“省内存”要准确理解为：**避免逐时间步保存完整状态，并把操作组织成向量和矩阵乘；不是说整个序列只需要一个标量。**
...
> **矛盾前置（任务→极限→失效→打破）**：任务——训练时把 $O(L)$ 步递归搬到 GPU 并行；极限——$M_t=I-\beta_tk_tk_t^{\mathsf T}$ 含跨步矩阵乘且不可交换，朴素前缀和失效；失效——逐 token 递推 $O(L)$ 顺序步吃不满 Tensor Core，$C=L$ 全并行又引入 $O(L^2)$ 临时量；打破——chunk-wise 以 $C=64/128$ 切块（可驻 SRAM），块内三角求解→GEMM（$O(LCd+Ld^2)$），块间仅传 $S_{[t]}$（$d\times d$，约 64KB @ $d=128$ FP16），顺序步从 $L$ 压到 $L/C$（$L=10k,C=64\Rightarrow10k\to156$ 步）。
...
graph LR
    ...
    C2 --> Out[拼接 O]
```
这张图讲的是 chunk-wise 的边界：块内把 $C$ 个 token 交给矩阵乘（GEMM，通用矩阵乘法），块与块之间仍然按时间顺序传递状态；它减少的是串行步数，不是假装状态依赖不存在。
#### 块内的 $w,u$ 变换
...
UT 指 unit triangular（单位三角）：主对角线全为 1、只有下半有值的矩阵，正好对应“只看本块内更早位置”的因果约束。如果逐行计算上面的 $w,u$，块内仍然像一个小型递归。论文把它写成下三角系统：
...
$$
这里的逆不是建议真的用通用矩阵求逆。$T$ 来自单位下三角矩阵，工程上可以用 forward substitution（逐行代入求解三角方程组）或对应的三角求解；论文的要点是把大部分工作改造成 GEMM（通用矩阵乘法，GPU 最擅长的大规模矩阵乘）和三角矩阵操作，让 Tensor Core 有活可做。
#### 块状态和块输出
...
主论文构建 DeltaNet layer 时，用 SiLU（一种激活函数，$x\cdot\sigma(x)$，$\sigma$ 为 Sigmoid）后的向量做 L2 归一化（把向量除以自己的长度，使范数正好为 1）：
...
论文的 DeltaNet layer 大体沿用 LLaMA 风格的 Transformer++ 布局（Transformer++：用 RMSNorm、SwiGLU 等改进搭配成的 Transformer 训练配方）：用 DeltaNet 替换 Self-Attention，保留 RMSNorm（均方根归一化层）和 SwiGLU FFN（门控线性单元激活的前馈网络）；$\beta_t$ 的额外参数量相对主投影很小。论文还在 Q、K、V 投影后加入 short depthwise convolution（短深度卷积：沿序列位置、每个通道各自独立做小窗口卷积），为局部位置变化提供补充。
...
| 硬件 | 340M 与 1.3B 实验使用 8 张 H100（英伟达数据中心 GPU）；吞吐图为单张 H100 | 论文 Appendix A.1、Fig.6 caption |
...
需要特别注明：Table 1 的一些 Transformer++、RetNet、Mamba（状态空间模型基准）、GLA（Gated Linear Attention，门控线性注意力）数字取自 Yang et al. 2023，DeltaNet 和 hybrid 是本文作者训练或重训的结果。表格可以做同框参考，但不能把所有数字误写成同一次完全独立重跑。
### 4.2 语言模型结果：DeltaNet 赢在哪，输在哪

先看论文 1.3B、100B tokens 的主结果。PPL（Perplexity，困惑度：模型给文本序列的平均概率越低，PPL 越高、效果越差）越低越好；SWDE、SQuAD、FDA 是准确率类指标，越高越好。这几个基准的含义：LAMBADA 给一段文字、遮住最后一个词让模型预测；SQuAD 给段落和问题做阅读理解；SWDE 从网页 HTML 中抽取结构化关系；FDA 从药品评审 PDF 中检索键值对（论文 §4.2 的定义）。
...
论文用 MQAR、MAD、RegBench 等合成任务测试“模型能不能从上下文中找回之前出现过的关联”。MQAR（Multi-Query Associative Recall，多查询关联回忆）在上下文里铺一段键值对应关系、再逐个位置考“之前存的值还在不在”；MAD（合成关联任务数据集，含压缩、模糊回忆、选择性拷贝等子任务）；RegBench（正则语言上下文学习：从示例中现场推断一门由概率有限自动机定义的构造语言）。其中 MQAR 的设置是 sequence length 512、key-value pairs 64，图中对比了 DeltaNet、Mamba、GLA、RetNet、RWKV4 和 Hyena。
...
还有第三条路线：不动状态，只砍掉每个 Query 要看的候选数。模块一的 [3.11 从 O(L²) 到 O(Lk)：DeepSeek 稀疏注意力 DSA 详解](/AIInfraGuide/prerequisites/模块一-前置知识/transformer/311-deepseek稀疏注意力dsa详解) 讲 DSA（DeepSeek Sparse Attention，稀疏注意力：先用轻量索引器给每个 Query 挑出最相关的 $k$ 个历史位置，只对这 $k$ 个位置做精确 Softmax，把主注意力复杂度从 $O(L^2)$ 压到 $O(Lk)$）——它和本文的分工是：DSA 保留精确 Attention 语义、用“少看”换复杂度；DeltaNet 换掉 Attention 语义、用“固定状态”换复杂度；两条路线的共同前提都是“不再让每个 Query 看全部历史”。
...
论文 §5.3 明确说，DeltaNet 的训练速度仍然落后于 GLA。原因不是渐进复杂度写错，而是状态到状态的依赖需要在 Kernel 内沿 head dimension 做 marginalization（在矩阵计算里沿该维度归约求和）；GLA 的 elementwise recurrence 更容易用 tiling 支持任意 head dimension。
这条限制会反过来影响状态大小：如果为了更强检索把 state 做大，状态依赖的 Kernel 代价也会变大，论文观察到 1.3B 规模下 DeltaNet 的 state size scalability 较弱。
#### 长度外推限制：没有显式 decay

论文还报告 DeltaNet 的 length generalization 有限，而 GLA、RetNet，以及一定程度上的 Mamba 能外推到超过训练长度的上下文。作者推测原因是 DeltaNet 的 recurrence 没有显式 decay factor（让旧状态随时间衰减的缩放因子），并把带 gating 的改进留作后续方向。
这里要区分事实和解释：
...
1. 序列混合层到底是 Delta Rule、gated linear attention、SSM（State Space Model，状态空间模型：一类把序列当作带隐状态的线性系统递归建模的方法）还是其他形式。
2. 状态的实际形状、精度、层间布局和是否需要额外 KV。
3. 训练 Kernel 是否使用本文的 UT/WY 变换，推理 Kernel 是否另有实现。
4. 端到端 PPL、检索质量、TTFT（首 token 延迟，用户等到第一个词的时间）、TPOT（每个输出 token 的平均生成时间）和吞吐是否在同一设置下比较。
## 📝 总结
...
- 能复算 §3.4 的 2×2 走查：同一 Key 两次写入时，$S_1M$ 先擦掉沿 $k$ 的第一列、$v_2k^{\mathsf T}$ 再写入新值，最终 $S_2q=[8,2]^{\mathsf T}$。
...
- [3.11 从 O(L²) 到 O(Lk)：DeepSeek 稀疏注意力 DSA 详解](/AIInfraGuide/prerequisites/模块一-前置知识/transformer/311-deepseek稀疏注意力dsa详解)（另一种打破 $O(L^2)$ 的办法：保留精确 Softmax，用索引器把每个 Query 的候选位置压到 $k$ 个）。
