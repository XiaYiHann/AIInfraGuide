...
<img src="/AIInfraGuide/images/flashattentionv1-0.png" alt="FlashAttention 分块计算示意图" style="max-width: 100%; display: block; margin: 0 auto;" />
...
💡 **注**：8192 的 OOM 取决于具体训练配置（batch × 头数 × 层数）：单个 `(batch, head)` 的 S/P 矩阵仅 $8192^2 \times 2\,\text{B} \approx 134\,\text{MB}$，单看不会 OOM；标准 Attention 要把全部 S/P 留在显存供反向使用，多 batch × 多头 × 多层累加后才会超出显存（论文 Table 2 的 OOM 记录即在该训练配置下，本站未复现，具体配置见原文）。
...
> ⚠️ **口径说明**：下表同时给出论文 kernel 与本站 Triton 简化实现的并行划分，两者一致：每个 program 负责一个 `(batch, head)` 切片的完整注意力。
>
...
1. **Thread Block 映射**：V1 中每个 Thread Block（program）负责一个 `(batch, head)` 切片的完整注意力，外循环遍历 K/V 块（即 $j$），内循环处理 Q 块
...
- ❌ **把“O(N²) 显存”和“O(N²) 计算”混为一谈，以为 FlashAttention 降低了计算量**：FA 只是把注意力矩阵不落显存，计算量依然是 $O(N^2)$。为什么错：把 IO 优化误当成计算优化，会错误地预期加速比与序列长度无关。正确做法：区分 HBM 访问（$O(N^2) \to O(N)$）与 FLOPs（不变）。
- ❌ **忽略分块必须适配 SRAM 大小**：把 $B_r \times B_c$ 设得很大“减少循环次数”。为什么错：块太大放不进 Shared Memory，要么报错要么悄悄 spill，反而更慢。正确做法：块大小按 `SRAM 容量 ≥ Br×d + Bc×d + Br×Bc` 的约束反推。
- ❌ **在 Decode 阶段照搬 Prefill 的分块配置**：batch=1、head=32 时 grid 只有 32 个 program。为什么错：SM 利用率只有约 30%，算力闲置（这是 V1 的主要短板）。正确做法：Decode 场景换用 Flash-Decoding 或 V2 的并行化思路，把 K/V 也切开并行。
- ❌ **把 Online Softmax 的修正因子丢掉**：分块计算时每块独立 softmax 后直接累加。为什么错：各块分母不同，直接相加的结果不是真正的注意力输出。正确做法：跨块合并时按 5.2 的递推式重缩放。
