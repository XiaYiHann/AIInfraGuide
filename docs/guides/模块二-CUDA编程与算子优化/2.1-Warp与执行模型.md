...
- ❌ **把 Warp 当作 32 个独立线程来设计分支**：写 `if (tid % 32 == 0) do_something();` 这类每 32 个线程只有 1 个工作的代码。为什么错：Warp 是锁步执行的，分支会让未命中的线程空转等待，吞吐直接除以 32。正确做法：让整个 Warp 走同一条路径，或把“只有 1 个线程要做的活”留到最后（如规约的 tid==0 收尾）。
- ❌ **用 `%` 和整数除法做线程编号映射**：`tid % 2 == 0` 这类判断在 CUDA 里没有编译器魔法，是真实的除法指令。为什么错：浪费指令周期，且常常是 Warp Divergence 的来源。正确做法：用位运算（`tid & 1`）或让编译器可展开的循环。
- ❌ **假设 blockDim 一定是 32 的倍数**：手动把 block 配成 100 之类的非 32 倍数。为什么错：最后一个 Warp 只有 4 个线程，剩下 28 个槽位空转，Occupancy 与代码逻辑都可能出错。正确做法：block 大小取 32 的倍数（128/256/512 最常见）。
- ❌ **把 “Warp 内线程同步” 与 “__syncthreads()” 混为一谈**：在 Warp 内也用 `__syncthreads()`。为什么错：多此一举浪费周期，且 `__syncthreads()` 是全 Block 屏障，条件分支里乱放会导致死锁。正确做法：Warp 内用隐式同步或 `__syncwarp()`，跨 Warp 才用 `__syncthreads()`。
