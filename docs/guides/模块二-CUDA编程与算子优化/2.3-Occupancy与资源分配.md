...
- ❌ **盲目追求 100% Occupancy**：把 block 配到最大、寄存器用到最少。为什么错：Occupancy 只是并行度上限，不是性能目标——寄存器太少会导致溢出（spilling）到局部内存，反而更慢；很多 Kernel 在 50% Occupancy 就带宽饱和了。正确做法：用 Occupancy Calculator 或 Nsight Compute 找“性能-资源”的拐点。
- ❌ **把 `__launch_bounds__` 的 minBlocksPerMultiprocessor 拍脑袋填**：为了凑 Occupancy 强制压低寄存器数。为什么错：寄存器被压到不足以装下关键数据时，编译器只能 spill，性能不升反降。正确做法：先看 Nsight Compute 的 Register Spill 指标，再决定是否限制寄存器。
- ❌ **共享内存不设上限就乱申请**：每个 block 申请 100KB Shared Memory 然后发现 Occupancy 掉到 1。为什么错：Shared Memory 是每 SM 的稀缺资源（A100 164KB），申请越多能同时驻留的 block 越少。正确做法：先算“每 block 共享内存 × 目标 block 数 ≤ SM 上限”，再决定分块大小。
- ❌ **用 block 数判断 Kernel 好坏**：看到“SM 都满了”就觉得优化到位。为什么错：Block 数多但每个 block 都在等内存，照样跑不满。正确做法：看实际指标——SM 占用、stall 原因、内存吞吐利用率。
