...
- ❌ **把 `tl.arange` 的长度当运行时参数**：`tl.arange(0, n)` 里的 n 是变量。为什么错：`tl.arange` 必须是编译期常量且长度为 2 的幂，运行期变量直接编译失败。正确做法：把长度设为 `tl.constexpr` 或在编译期确定。
- ❌ **在 Triton Kernel 里写 Python 分支**：用 Python `if x > 0` 判断张量数值。为什么错：Triton 里 Python 分支是编译期控制流，不能依赖张量数值；数值分支要用 `tl.where` 或 `tl.min/max`。正确做法：数值条件一律用 `tl.where` 表达。
- ❌ **忘记 `tl.constexpr` 与运行时参数的区别**：把 block 大小传成普通参数，然后惊讶“为什么每次启动都重新编译”。为什么错：普通参数变化会触发重新编译（JIT 缓存按签名区分），性能抖动。正确做法：固定形状用 `constexpr`，把编译实例数控制在几个。
- ❌ **假设 Triton 自动处理一切边界**：数组长度不是块大小的整数倍时直接越界读。为什么错：Triton 的掩码（mask）要自己写，不掩码就会读越界（可能读到垃圾或崩溃）。正确做法：尾部块显式传 mask 或 `other` 填充值。
