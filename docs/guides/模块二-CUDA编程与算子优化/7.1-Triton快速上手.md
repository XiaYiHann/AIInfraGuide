...
📌 **关键点**：单独一个 Softmax，Triton 相对 PyTorch 的收益有限——官方教程的基准（M=4096，N 从 256 扫到 12k+，以 GB/s 计）显示：**N 较大时 Triton 与 `torch.softmax` 相当甚至略快，N 较小时 PyTorch 更快**（教程原文也只承诺“significantly faster than PyTorch's native op”的前提是“行能装进 SRAM”，且对比对象是朴素的多次遍历实现）。**Triton 的真正价值是“融合”**：把 softmax 前后的 elementwise 算子合并进同一个 Kernel，省掉中间张量的写 + 读。以 $(M, N) = (4096, 4096)$ FP16 为例，一个中间张量就是 32 MB，省一次写 + 一次读 = **64 MB HBM 流量**，这对 Memory-Bound 场景是实打实的收益。

调参两个编译期 hint（以你所用版本的 `--help` / 文档为准）：
...
- **调试难度**：你写的 Python 是一层"声明"，真正的代码是编译器生成的 IR/PTX——报错信息经常指向你看不懂的 MLIR 片段
