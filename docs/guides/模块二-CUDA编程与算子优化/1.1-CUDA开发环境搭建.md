...
- ❌ **驱动和 Toolkit 版本对不上就开跑**：`nvcc --version` 显示 12.x，`nvidia-smi` 显示驱动只支持 11.x。为什么错：Toolkit 的 PTX/JIT 需要驱动支持对应 CUDA 版本，版本不匹配编译能过、运行报 `CUDA driver version is insufficient`。正确做法：先查驱动支持的 CUDA 版本（`nvidia-smi` 右上角），再装 ≤ 该版本的 Toolkit。
- ❌ **用 `.run` 包安装时不关 nouveau/不进 tty**：图形界面下直接装驱动。为什么错：nouveau 驱动冲突会导致安装失败或黑屏。正确做法：先禁用 nouveau、切到文本模式再装（本文 1.2 有完整步骤）。
- ❌ **把 `/usr/local/cuda` 的软链指向随意换**：多版本共存时直接删软链。为什么错：系统里其他依赖 `/usr/local/cuda` 的库会全部失效。正确做法：用 `update-alternatives` 或显式 `CUDA_HOME` 管理，不删软链。
- ❌ **`-arch=sm_XX` 与 `-compute_XX` 混用**：编译选项写错计算能力。为什么错：sm_XX 生成 SASS（绑定架构），compute_XX 生成 PTX（可 JIT）；填错会出现运行期 Illegal Instruction 或性能退化。正确做法：明确目标卡的计算能力（A100=sm_80，H100=sm_90），用 `-arch=sm_80` 或 `-gencode` 组合。
...
- 能用 `nvidia-smi` 与 `nvcc --version` 分别确认驱动与 Toolkit 版本，并说明两者版本不匹配时的典型报错特征
- 能说出 `-arch=sm_XX` 与 `-compute_XX` 的区别，并解释为什么编译时指定错误的计算能力会得到运行期 Illegal Instruction 错误
