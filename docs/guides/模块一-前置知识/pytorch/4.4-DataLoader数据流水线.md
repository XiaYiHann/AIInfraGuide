...
> 💡 **状态机一瞥（DataLoader 流水，图中每条箭头对应一行状态迁移，Shape 守恒）**
> | 状态（持有） | 事件 | 动作 | 新状态 |
> |---|---|---|---|
> | Sampler 待产 | BatchSampler 分组 | 产 $[B]$ 索引组 | 索引组就绪 |
> | worker 空闲 | 取 $[B]$ 索引 | $\{id\to tokens\}$ 读取+变换 | 样本列表 $[B, S_{var}]$ |
> | 样本列表 | collate_fn 触发 | pad 至 $[B,S_{max},H]\times2$B 驻主机内存 | Batch $[B,S_{max}]$ 就绪 |
> | Batch 就绪 | 入队 | 经队列到主进程，pin 后 HBM 可直接 DMA | 主进程队列 |
> 图中 D→W 的箭头对应第 2 行“取索引”，W→C 对应第 3 行 collate，C→Q 对应第 4 行入队；全程主机内存为 Byte 口径，HBM 为目标。
...
若设置 `drop_last=True`，最后的 `[1]` 被丢弃。对于 BatchNorm、静态 shape 或要求每个 rank（分布式进程编号 $0\ldots N-1$，每卡一 rank）步数一致的训练，这种整齐 batch 可能值得；代价是训练样本被丢弃。评估集通常应保留尾批，否则指标分母会悄悄变化。
...
它仍是 Sampler：作用是让每个 rank（分布式进程编号 $0\ldots N-1$，每卡一 rank，DistributedSampler 按 rank 把数据集切成 $N$ 份、每 rank 各取 1/N）只看到 Dataset 的一个索引子集。下面只建立入口，不启动进程组：
...
例如 $W=4,P=2$，上限为 8 个 batch。若每个 collated batch 占 64 MiB，仅 payload 粗算就是 $8\times64=512$ MiB（Byte 口径，驻主机内存；同等 Batch 搬至 HBM 3.35TB/s 下约 0.019 ms/Batch，PCIe 5.0 64GB/s 下约 1.0 ms/Batch）；这还没包含 worker 副本、Python 对象、队列和临时解码内存。**预取用主机内存换取消费者暂时不等数据，队列已经够满时继续增加没有免费收益。**
...
普通 pageable 内存像可以被操作系统临时换走的座位，CUDA 在搬运前可能要先把数据放到稳定的中转区；pinned（page-locked）内存则不会被换页，DMA 可以直接使用。**`pin_memory=True` 让 DataLoader 返回的 Tensor 位于页锁定内存（主机 pinned 内存→HBM 的 DMA 直通，省一次 pageable→pinned 拷贝），但它只准备了传输条件，不证明端到端更快。**  // 量级：64 MiB Batch 在 PCIe 5.0 单向 64 GB/s 下 H2D 约 1.0 ms，HBM 带宽 3.35 TB/s 下设备内搬运仅 0.019 ms；是否重叠需 4.6 的 Stream/Event 实测。
...
分工：4.3 管“模型拥有什么”，4.4 管“batch 怎样来”，4.5 管“batch 怎样驱动可恢复训练”，未来数据系统管“数据怎样在集群中可靠地产生与送达”。
