...
区别：**`torch.compile` 优化当前 Python callable 的运行，`torch.export` 交付一份带输入约束的 AOT `ExportedProgram`。**
...
分工：**4.9 管 operator 契约，4.10 管单进程执行/编译地图，4.11 管多进程入口，模块二才管编译器与 Kernel 怎样做深。**
