...
这张图讲的是**传递的约定顺序**：每张卡只跟左右两个邻居打交道，数据沿环单向流动。就像全班同学手拉手围成一圈传纸条，你只需要“从左边同学手里接过来、递给右边同学”——不需要认识圈子里的每个人。
...
数一数就能得到公式。每张卡的数据被切成 N 片，每片 $M/N$ 字节；ReduceScatter 阶段做 N-1 轮，每轮发一片，所以发 $(N-1) \times \frac{M}{N}$ 字节；AllGather 阶段同样再发 N-1 片。合计：
