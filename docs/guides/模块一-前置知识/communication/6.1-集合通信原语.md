...
多卡训练多了一个全新的成本项——**卡间搬运**。按“任务需求→物理极限→旧方案失效→新方案”四句链看：**任务**是千亿参数训练必须用多卡，**极限**是 NVLink 900 GB/s / IB 50 GB/s 的带宽墙，**旧方案**点对点 Send/Recv 要手写 $O(N^2)$ 配对，**新方案**把高频搬运抽象成五种标准原语——这就是本章要回答的。三个最常见的协作场景：
...
> 💡 **动态状态机（叠加）：Ring AllReduce 的 2(N-1) 步变迁（N=4）**
> | 步骤 | 状态（谁持有什么） | 事件 | 动作 | 新状态 |
> |---|---|---|---|---|
> | 0 | rank0:[a0,b0,c0,d0] … rank3:[a3,b3,c3,d3] | 开始 Scatter-Reduce | 每卡把一块发给下游，叠加收到的块 | 每卡各有一块的全局和 |
> | 1–3 | 每卡各持一块全局和 | 循环 N-1 轮 | 环上顺时针传递+累加 | ReduceScatter 完成：rank0:sum(a), rank1:sum(b)… |
> | 4–6 | 每卡各持一块全局和 | 开始 AllGather | 环上顺时针转发完整块 | AllGather 完成：每卡持 [sum(a),sum(b),sum(c),sum(d)] |
> 每步每卡发送量均为 $M/N$，总计 $2(N-1)M/N\approx2M$，与表 5.2 一致；N=1 时退化为 0 步本地操作。图中每个箭头对应一行状态迁移。
...
> 📌 **三行收束（叠加）——本章五原语的取舍**：
> - **收益**：Broadcast/Reduce 解决“1↔多”分发/汇总，AllReduce/AllGather/ReduceScatter 解决“多↔多”协作，语义与并行策略一一对应；
> - **代价**：AllReduce 每 rank 链路代价 $\approx2M$ 最贵，AllGather/ReduceScatter 约 $M$ 便宜一半，但 ZeRO-3 频率高（每层前后各一次）把便宜变贵；
> - **边界**：数据 < few KB 时延迟主导，不用算带宽账；N 很大时 AllGather 逼近 $M$ 与集群规模解耦，适合千卡以上；TP 的 AllReduce 必须在 NVLink 域内，否则跨机 50 GB/s 会被卡死。
