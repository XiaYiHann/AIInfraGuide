...
与之对应的是**非自回归生成**（Non-Autoregressive Generation，NAR）：不要求 token 严格从左到右落位，而是允许多个位置同时更新。LLM 时代最值得关注的新形态是 masked diffusion（掩码扩散语言模型）：先随机遮住 token，再逐步把这些空位填回来。

当前主流大语言模型（Large Language Model，LLM）仍以 AR（Autoregressive，自回归）为主；但 NAR 已不只用于质量要求可以适度放松的机器翻译，LLaDA（Large Language Diffusion with mAsking，一种从头训练的掩码扩散模型）展示了另一条 LLM 生成路径。

**前向过程：按掩码率 $t$ 随机挖空。** `[MASK]` 是“这里暂时看不见”的占位符。训练时先从 0 到 1 之间随机抽一个掩码率 $t$，再让每个 token 以概率 $t$ 被替换成 `[MASK]`；可以把这个过程写成：

$$
t\sim U[0,1],\qquad \Pr(x_i=\text{[MASK]})=t
$$

这里 $U[0,1]$ 表示在 0 到 1 之间均匀抽样，$x_i$ 是第 $i$ 个 token。于是 $t=0$ 近似表示一个 token 都不遮，$t=1$ 表示全部遮住，$t=0.5$ 则表示每个位置有一半概率被遮住；实际遮住多少个位置会因随机抽样而波动。

**反向过程：并行填空，再把低置信位置重新遮住。** 推理时可以从一段给定长度的全 `[MASK]` 响应开始。每一步，模型同时预测所有被遮住的位置；如果某个位置的预测置信度低，就做 remask（重新遮住：把刚预测的 token 变回 `[MASK]`），这个动作也叫 remasking，然后在下一步重新预测。

最小走查（思想实验）：把句子 `今天 | GPU | 已经 | 跑 | 满 | 了` 当作 6 个教学 token，随机遮住 3 个，得到 `今天 | [MASK] | 已经 | [MASK] | [MASK] | 了`。第 1 步同时预测 3 个空位；假设前两个位置置信度高、第三个位置置信度低，就只把第三个位置 remask，再在第 2 步重新预测；如果仍不稳定，第 3 步再修一次。这个例子展示了“1 次并行填 3 个位置，随后只重做不确定位置”，在玩具设置下可用 2-3 步收敛。

**把两种生成的成本账放在一起。** 下面的 AR 指自回归生成；LLaDA 侧采用论文设置的 vanilla MHA（vanilla Multi-Head Attention，标准多头注意力：每个注意力头都有独立的 K、V），不使用 GQA（Grouped-Query Attention，分组查询注意力：多个 Query 头共享一组 K、V）和 KV Cache（把历史 K、V 缓存在显存中以供后续复用）。Prefill 指“一次把整段序列送进模型”的全序列前向；compute-bound（计算受限）指时间主要花在矩阵乘法等算术运算，而不是等待数据搬运。表中的 reversal curse（反转诅咒）指模型能记住 $A\rightarrow B$，却不容易从 $B$ 反推出 $A$。

| 成本维度 | AR（自回归） | masked diffusion（以 LLaDA 为例） | 来源口径 |
|---|---|---|---|
| 注意力方向 | 因果单向：当前位置只能看左侧上下文 | 双向注意力：每个位置可同时利用左右上下文 | [LLaDA 论文](https://arxiv.org/abs/2502.09992) §1、Figure 2 |
| KV Cache | 高性能逐 token 生成通常缓存历史 K、V，减少前缀重复计算 | 不使用 KV Cache；每一步重新对当前全序列做前向 | LLaDA 论文 §2.2；课程幻灯片“Pre-training Setup” |
| 单步前向 | 每次追加 1 个 token，下一步依赖上一步的结果 | 每一步并行预测所有 masked token，再选择位置 remask | LLaDA 论文 §2.4；课程幻灯片“Inference” |
| 硬件倾向 | 小 Query 反复读取越来越长的历史，常受显存访问影响 | 全序列矩阵运算占比高，工程上更偏 compute-bound，形态类似 Prefill | 机制对照；LLaDA 的标准 Transformer 设置 |
| 步数与速度/质量 | 每生成一个新 token 都要完成一次有依赖的决策，不能把整条因果链跳过 | 16-256 步是可调的速度/质量旋钮，改变步数不需要重新训练 | 课程幻灯片“Inference”；论文 §2.4 的灵活采样 |
| 规模与对照 | — | LLaDA-8B 在 2.3T（2.3 万亿）个 token 上训练，并与 LLaMA3-8B 对比 | LLaDA 论文 §1、§3 |
| 反转提问 | 左到右的条件链有方向偏置 | 双向上下文有助于缓解 reversal curse；例如见过 `the cat is on the mat` 后，从 `the mat` 反推出 `the cat` | LLaDA 论文 §1；解释性例子 |

📌 **关键点**：masked diffusion 省掉的是严格的逐 token 串行依赖和 KV Cache，不是每一步的全序列计算；它把“多次小的逐 token 前向”换成“少量多次的全序列前向”。

扩散步数和 AR 步数不是同一种取舍：AR 每生成一个 token 就增加一个通常不可跳过的因果步骤；diffusion 一步可以并行更新多个位置，步数是可调的质量旋钮。少步更快但可能牺牲质量，多步更稳但要重复全序列前向，而且这个旋钮可以在不重训的情况下调整。

这也说明它与站内 6.x 推理优化的分工：双向注意力和无 KV Cache 减少了缓存与因果约束，但每个扩散步都像一次全序列 Prefill，不能直接套用只针对逐 token Decode（解码阶段：每次只生成一个新 token）的 KV Cache 优化。
...
PagedAttention 的效果非常显著：论文（Kwon et al., 2023）报告现有系统只有 20.4%-38.2% 的 KV Cache 显存真正存放 token 状态（即浪费约 60-80%），PagedAttention 把它降到接近零（近零浪费），配合分页管理让 vLLM 在相同显存下吞吐提升 2-4 倍（口径见 PagedAttention 论文 §3.1 与摘要，本站未复现，具体数值以论文为准）。
...
“attention sink”现象（开头 token 的 KV 不能随便丢）的系统性研究见 [2.6 StreamingLLM 与 Attention Sink](/AIInfraGuide/inference/模块四-推理优化/第2章-推理引擎核心技术/26-streamingllm与attention-sink)：naive 滑窗为何在首 token 出缓存瞬间 PPL 爆炸，以及“N 个永不驱逐的 sink token + 最近窗口”的修法。
...
- ❌ **不用 KV Cache，每步重算整个前缀**：把全部历史 token 重新喂进模型。为什么错：生成第 n 个 token 要重算 $O(n^2)$ 的注意力，慢得不可用（本文对比了无 Cache 的代价）。正确做法：缓存每层的 K/V，每步只算新 token 的注意力。
- ❌ **用均值延迟衡量生成体验**：只报平均 TTFT/TPOT。为什么错：均值被少数快请求拉低，用户感知的是尾延迟（P95/P99）；长尾请求卡顿时均值依然好看。正确做法：用分位数指标（P95/P99）评估，见 9.1。
- ❌ **把采样温度当“正确性开关”乱调**：为追求多样性把温度调得很高。为什么错：温度只改变分布形状，过高的温度让输出随机化、事实性下降。正确做法：按任务选解码策略（事实问答用低温度/greedy，创意生成用适度温度 + top-p）。
- ❌ **忽略 Prefill 与 Decode 的负载差异**：把整条链路当成均匀负载来优化。为什么错：Prefill 是计算密集、Decode 是访存密集，混在一起会互相拖累（尾延迟爆炸）。正确做法：按两阶段分别分析瓶颈（这是 PD 解耦的动机，见模块四第 7 章）。
