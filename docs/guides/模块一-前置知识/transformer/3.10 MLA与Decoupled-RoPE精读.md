---
title: "3.10 KV Cache 为什么能省 93.3%？MLA 低秩压缩与解耦 RoPE（DeepSeek-V2 §2.1 精读）"
description: "拆解 DeepSeek-V2 的 Multi-head Latent Attention：KV 低秩联合压缩、训练期 Q 压缩、Decoupled RoPE，算清 KV cache 省 93.3% 与吞吐 5.76 倍的账"
pubDate: 2026-08-19
...
KV Cache 的大头不是模型参数，而是 Decode 时反复从显存读回的历史 Key 和 Value。DeepSeek-V2 的 MLA（Multi-head Latent Attention，多头潜在注意力）把它们联合压进一个 latent（潜向量，把完整内容压缩成的低维表示），再用一条很窄的 RoPE 位置通道补回相对位置信息；论文报告与 DeepSeek 67B 相比 KV cache 减少 93.3%，在 8 张 H800（英伟达 Hopper 架构数据中心 GPU）上最大生成吞吐达到 5.76 倍，但这两个数字还叠加了实际部署的 6-bit 平均 KV 量化，不能误读成“某个公式必然带来 93.3%”。
...
- [📚 参考资料](#-参考资料)
## 🗺️ 原文阅读地图
这篇文章精读 DeepSeek-V2 的注意力部分，不把整篇 MoE（Mixture-of-Experts，混合专家：让每个 token 只激活少数几个专家子网络、从而扩大容量的架构）论文重新讲一遍。
...
**你只需要沿着一条数据流走：当前 token（词元，模型处理文本的最小单元）的隐藏状态 `h_t`，经过压缩，变成可缓存的状态；下一步 Decode 只读取这些状态。**

在标准自回归生成中（每生成一个词，就把它接回历史、再据此预测下一个词），当前 token 产生 Query，历史 token 提供 Key 和 Value。
...
### 0.3 你需要的三个前置
第一，Attention 的内容路径可以写成“Query 和 Key 算相关性，再用权重汇总 Value”──这套 Q、K、V 投影与权重归一化的完整来龙去脉见站内 [3.3 Self-Attention 机制深入理解](/AIInfraGuide/prerequisites/模块一-前置知识/transformer/33-self-attention机制深入理解)，本文直接沿用这条内容路径，只在其上做 MLA 的结构性改动。

第二，KV Cache 是把历史 Key、Value 存起来，避免每个 Decode step 重新计算前缀的投影。

第三，RoPE（Rotary Position Embedding，旋转位置编码：把“位置差”通过旋转矩阵编码进 Q 与 K 的夹角）会把位置编码进 Q、K 的几何关系；完整几何推导见站内 [3.5 Transformer 位置编码深入理解](/AIInfraGuide/prerequisites/模块一-前置知识/transformer/35-transformer位置编码深入理解)，本文只保留 MLA 需要的那一步。
...
> 💡 **Roofline 一瞥（叠加，估算口径）**：Decode 每步算术强度 $I\approx\frac{2d}{2\cdot d\cdot b_e}\approx0.5$ FLOP/Byte（一次 Query 与 $S$ 个历史状态各做内积，访存主导），远低于 H800 的 Roofline 拐点（~ 989 TFLOPs / 3.35 TB/s $\approx$ 295 FLOP/Byte），属于典型的**访存受限（Memory-Bound）**；Prefill 批处理大矩阵 $I$ 可上百，为**算力受限**。所以 MLA 优先压“访存边”而非“计算边”，这解释了为什么论文在 Decode 吞吐上收益最大。
...
> 💡 **Shape 与驻留（叠加）**：$M_{\mathrm{KV}}$ 本身不改变形状，它算的是历史状态在 **HBM**（GPU 高带宽主显存，存放全量 KV Cache）中的字节数；计算时每次只把当前 Query（$d$ 维）与历史 $S$ 个状态搬到 **SRAM**（片上高速缓存）做内积，搬运量随 $S$ 线性增长——这正是 §1.1 要钉死的“带宽墙”。

用一个已有路线中常见的算例检查数量级：$l=32$、$n_{\mathrm{kv}}=32$、$d_h=128$、$S=4096$、$B=16$、FP16（半精度浮点数，每个数字占 2 字节）的 $b=2$。
$$
...
标准 MHA 经过三个投影得到（**Shape 守恒**：$\mathbf{h}_t\in\mathbb{R}^{d}$ 经过 $W^Q,W^K,W^V\in\mathbb{R}^{(n_hd_h)\times d}$ 分别得到 $\mathbf{q}_t,\mathbf{k}_t,\mathbf{v}_t\in\mathbb{R}^{n_hd_h}$，再按 $n_h$ 切头成 $[n_h, d_h]$——投影本身不改变 batch/序列维度，只改变每 token 的表示宽度）：
$$
...
先解释标题里的“低秩”。线性代数里的“秩（rank）”量的是一个矩阵中真正独立的信息条数；**低秩（low-rank）压缩的直觉是：很多高维表示里的信息其实是冗余的，用一个维度小得多的中间向量就能装下绝大部分内容**——类似一张大照片，压成很小的缩略图也足够让人认出主体。MLA 的 $d_c=512$ 就是这颗“缩略图”的宽度，用它去近似完整 MHA 要存的 32768 维 K、V。

**步骤 1：把隐藏状态压到 KV latent（Shape：$\mathbb{R}^{d}\to\mathbb{R}^{d_c}$，$512\ll32768$）。**
$$
\mathbf{c}_t^{KV}=\underbrace{W^{DKV}\mathbf{h}_t}_{\text{KV 下投影,得到共同 latent}}.
$$
$\mathbf{c}_t^{KV}\in\mathbb{R}^{d_c}$ 是要缓存的低维状态，$W^{DKV}\in\mathbb{R}^{d_c\times d}$ 是下投影矩阵；**驻留**：$\mathbf{c}_t^{KV}$ 常驻 HBM，Decode 时只把当前 $\mathbf{q}_t$ 与历史 $S$ 个 $\mathbf{c}_j^{KV}$ 搬到 SRAM 做相关性计算。

如果 $d_c\ll n_hd_h$，那么每个历史 token 的主要缓存宽度就明显变小。

**步骤 2：沿 Key 路径上投影（Shape：$\mathbb{R}^{d_c}\to\mathbb{R}^{n_hd_h}$，$512\to16384$ 的“展开”操作，训练/吸收路径用，历史不存展开结果）。**
$$
...
**步骤 3：沿 Value 路径上投影（Shape：$\mathbb{R}^{d_c}\to\mathbb{R}^{n_hd_h}$，同上，共享输入 latent）。**
$$
...
“吸收（absorb）”在这里的意思是：因为矩阵乘法满足结合律，$(a^T W)c = a^T(Wc) = (W^T a)^T c$，所以可以把 $W^{UK}$ 先乘进当前 Query 那一侧，历史端就只剩 latent 与合并后的路径做内积——把“逐历史 token 恢复完整 Key”这件麻烦事吸收（合并）进 Query 侧。
...
把它一步步写开：利用转置恒等 $(\mathbf{q}_t^C)^T W^{UK} = (W^{UK,T}\mathbf{q}_t^C)^T$，所以 $(\mathbf{q}_t^C)^T\mathbf{k}_j^C = (W^{UK,T}\mathbf{q}_t^C)^T \mathbf{c}_j^{KV}$。Decode 时 $W^{UK,T}\mathbf{q}_t^C$ 只对“当前这一个新 token”算一次，历史端只需给出 $\mathbf{c}_j^{KV}$——这就是论文说的把 $W^{UK}$ 吸收进 Query 路径（等价于把它并入 $W^Q$）。
...
> 💡 **动态状态机（叠加）——Decode 一步的状态变迁**：
> | 步骤 | 状态（HBM 常驻） | 事件 | 动作（SRAM 内） | 新状态/输出 |
> |---|---|---|---|---|
> | 1 | 历史 $\{\mathbf{c}_j^{KV},\mathbf{k}_j^R\}_{j<t}$ | 到达新 token $h_t$ | 下投影得 $\mathbf{c}_t^Q$，算 $\mathbf{q}_t^C,\mathbf{q}_t^R$ | 当前 Query 就绪 |
> | 2 | 同上 + 当前 Query | 相关性计算 | $q^C$ 与 $c_j^{KV}$、$q^R$ 与 $k_j^R$ 做内积 | 得 $S$ 个分数 |
> | 3 | 同上 | 归一化 | Softmax 得权重 $\alpha_j$ | 权重就绪 |
> | 4 | 同上 | 加权汇总 | $\sum\alpha_j\mathbf{c}_j^{KV}$ 再经 $W^OW^{UV}$ | 输出 $u_t$，并追加 $c_t^{KV},k_t^R$ 进 HBM |
> 失败分支：若把普通 RoPE 误插进内容路径，步骤 2 的固定吸收路径失效，历史 $k_j$ 需重算，HBM 缓存语义被破坏——这正是 §3.5.2 的第一个失败条件。
...
$$
Softmax 的作用是把一排相似度分数归一化成总和为 1 的权重（分数越高权重越大），所以这里先算 Q、K 的内积，再用 Softmax 加权汇总 Value。
...
- **参数精度**：先把 DeepSeek-V2 的参数转换为 FP8（8 位浮点数格式）。
...
为了确认 93.3% 这个位数量级到底从哪来，可以按公开模型配置把两边从元素数换算成字节重算一遍（这是文章核算，用来核对数位量级，不是论文给出的分解式）：

- **DeepSeek 67B 侧**（对照模型，公开配置文件为 95 层、8 个 KV 头、head_dim=128）：每 token 的 KV 元素数 $2\times8\times128\times95=194{,}560$；假设按 FP16（2 字节/元素）计，约 $389{,}120$ 字节。
- **DeepSeek-V2 侧**：每 token 缓存 $(d_c+d_h^R)l=(512+64)\times60=34{,}560$ 个元素；按论文的平均 6 bit（0.75 字节/元素）计，约 $25{,}920$ 字节。
- **比值**：$1-\dfrac{25{,}920}{389{,}120}=1-0.0666\approx93.3\%$。

拆分看：只算元素数，MLA 相比 67B 大约省 $1-\tfrac{34{,}560}{194{,}560}\approx82\%$；再乘上 6-bit（0.75 字节）对 FP16（2 字节）约 2.67 倍的字节压缩因子，才把总节省推高到 93.3%。这也再次说明 93.3% 是“MLA 结构压缩 + KV 量化 + 具体模型配置”共同的结果，不是单靠某个公式就能复现的常数。
### 4.3 5.76 倍吞吐也有边界
...
MLA 的这套“latent 缓存 + 位置通道”基座也直接长出了后续 DeepSeek-V3 的稀疏注意力：在 latent 之上再选 top-k 做内容寻址，把主注意力从 $O(L^2)$ 降到 $O(Lk)$，见站内 [3.11 DeepSeek 稀疏注意力 DSA 详解](/AIInfraGuide/prerequisites/模块一-前置知识/transformer/311-deepseek稀疏注意力dsa详解)──它讲解 DSA 机制，本文只讲到 MLA 基座，DSA 是这套基座上的稀疏选摘。
...
- [3.11 DeepSeek 稀疏注意力 DSA 详解](/AIInfraGuide/prerequisites/模块一-前置知识/transformer/311-deepseek稀疏注意力dsa详解)：在 MLA 的 latent 缓存之上用 Lightning Indexer 选 top-k，把主注意力从 $O(L^2)$ 降到 $O(Lk)$；本文讲清 MLA 这套“低秩缓存 + 位置通道”基座，DSA 是这套基座上的后续演进。
- [3.12 MoE 论文精读（Sparsely-Gated MoE）](/AIInfraGuide/prerequisites/模块一-前置知识/transformer/312-moe-sparsely-gated-moe论文精读)：讲 DeepSeek-V2 另一半架构（DeepSeekMoE）的源头——路由与专家坍缩；本文特意不展开 MoE、只聚焦 MLA 注意力，想补 MoE 一侧见这一篇。
