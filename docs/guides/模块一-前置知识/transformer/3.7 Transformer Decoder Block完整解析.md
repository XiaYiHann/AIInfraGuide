...
✳️ 总结一句话：**Decoder-only 在大规模场景下，以最简洁的架构获得了最好的通用能力，同时对工程系统最友好。**

> 💡 **三账本一瞥（LLaMA2-7B，$B=4,S=2048,H=4096$）**
> | 账本 | 数量级 | 落点 |
> |---|---|---|
> | 计算 | 前向 $2Ns\approx2\times6.7B\times2048\approx27$T FLOPs/样本 | Tensor Core，HBM→SRAM |
> | 内存 | 权重 13.4GB (FP16·2B) 驻 HBM，KV Cache $2Lh_{kv}d_kS\times2$B$\approx0.5$GB/$seq$ | HBM 常驻 |
> | 通信 | 单机 $0$；TP 时每层 AllReduce $2BSH\times2$B$\approx0.07$GB 走 NVLink | NVLink 域内 |
> 估算口径：Batch 4 时权重搬运 13.4GB/2TB/s≈6.7 ms 决定 Decode 延迟，算术强度 $<$5 属访存受限。 从 GPT-3 到 LLaMA，再到 Mistral、Qwen、DeepSeek，全部采用这一路线。
...
> 💡 **状态机一瞥（Pre-Norm Decoder Block 前向，图中每个方块对应一行迁移，Shape 守恒）**
> ```mermaid
> flowchart LR
> A[输入:B,S,H<br/>HBM] --> B[RMSNorm<br/>SRAM]
> B --> C[QKV投影 H→H<br/>HBM→SRAM]
> C --> D[Attention B,S,H<br/>SRAM]
> D --> E[残差Add<br/>SRAM]
> E --> F[FFN H→4H→H<br/>SRAM]
> F --> G[输出:B,S,H<br/>写回 HBM]
> ```
> 图中 RMSNorm→QKV 的第 2 箭头对应正文“QKV 投影”第 2 步，Attention→残差的第 4 箭头对应第 4 步；全程 $BSH\times2$B 驻 HBM，SRAM 内分头完成 $QK^\top$。
...
**🍎 GQA 的引入**。LLaMA-2-70B 和 Mistral-7B 使用了 Grouped-Query Attention：KV 头数少于 Q 头数。LLaMA-2-70B 用 8 个 KV 头服务 64 个 Q 头（每组 8 个 Q 头共享 1 组 KV），这将 KV Cache 从 MHA 的 $2\times32\times128\times2$B$=1$KB/token 降至 $2\times8\times128\times2$B$=0.25$KB/token，降低 4×（$S=4096,B=16$ 时 32GB→8GB，Byte 口径）。
...
1. **架构收敛**：Decoder-only 成为主流——预训练目标统一、因果掩码天然适配自回归生成、工程上不需要 Encoder 的额外复杂度。
2. **数据流**：Embedding → 多头注意力（带因果掩码）→ 残差 + 归一化 → FFN → 残差 + 归一化，每个 block 输入输出形状不变（batch, seq, hidden）。
3. **参数量手算**：7B 级模型的参数量 = Embedding + 每层（Attention 4 个矩阵 + FFN 3 个矩阵）× 层数 + LM Head，能用通用公式手算到误差 5% 以内。
4. **计算量估算**：前向 FLOPs ≈ 2 × 参数量 × token 数；训练约 3 倍（前向 + 反向），推理每生成一个 token 都要跑一次 Decoder 前向。固定算力预算下参数量与 token 数怎么分配（约 20 tokens/parameter、推翻“大模型优先”的 Kaplan 结论）见 [chinchilla-notes](/AIInfraGuide/papers/chinchilla-notes)——该篇管“给定算力训多大”，本节只算“跑一遍要多少算力”。
5. **显存规划**：训练态 = 权重 + 梯度 + 优化器状态 + 激活；推理态 = 权重 + KV Cache——KV Cache 随 batch × 长度线性增长，是长上下文推理的主要显存项。

## ⚠️ 常见错误做法

- ❌ **手算参数量时漏掉 Embedding 或 LM Head**：只算 Attention + FFN。为什么错：7B 模型里 Embedding + LM Head 约 0.7B，漏算后显存账本对不上。正确做法：用本文的通用公式 $P = Vh + L \times (\text{attn} + \text{ffn}) + Vh$ 全量手算。
- ❌ **把训练 FLOPs 当成前向 FLOPs 用**：估算训练成本时只乘了 2。为什么错：训练 = 前向 + 反向 ≈ 3 倍前向 FLOPs（反向约 2 倍）。正确做法：前向 ×3 才是单步训练的开销。
- ❌ **推理显存只算权重**：忽略 KV Cache。为什么错：长序列 + 大 batch 时 KV Cache 可以超过权重（见 3.8 的账本）。正确做法：推理显存 = 权重 + KV Cache，KV 随 batch × 长度线性增长。
- ❌ **假设所有 Decoder-only 模型结构一样**：用 LLaMA 的结构套所有模型。为什么错：GQA/MLA、SwiGLU/RMSNorm、QK-Norm 等变体会改变参数量与 KV 大小。正确做法：先查目标模型的配置（heads、中间维度、是否共享 KV）再手算。
