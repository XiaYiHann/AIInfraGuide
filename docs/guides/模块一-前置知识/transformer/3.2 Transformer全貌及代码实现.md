...
> 💡 **三账本一瞥（LLaMA2-7B，$B=8,S=2048,H=4096,V=32000$）**
> | 账本 | 数量级 | 落点 |
> |---|---|---|
> | 计算 | 单层 $8BSH^2\approx0.22$T FLOPs，32 层≈7T FLOPs | Tensor Core GEMM，HBM→SRAM |
> | 内存 | 权重 14GB (FP16·2 Byte) 驻 HBM，激活峰值 $LSH\times2$B$\approx4$ GB，KV Cache $2LSH\times2$B | HBM 常驻、SRAM 复用 |
> | 通信 | 单机 $0$ Byte；张量并行时每层 AllReduce $2BSH\times2$B$\approx0.13$ GB 走 NVLink | NVLink 域内 |
> 估算口径：前向一次约 7T/1GB 量级，Decode 单 token 则访存强度 $<$5 FLOP/Byte 属访存受限，Prefill 属算力受限。
...
> 💡 **状态机一瞥（Encoder 层前向，图中每个方块对应一步迁移）**
> ```mermaid
> flowchart LR
> A[输入 X:B,S,H<br/>HBM] --> B[Self-Attn QKV投影<br/>HBM→SRAM]
> B --> C[残差+Norm<br/>SRAM]
> C --> D[FFN H→4H<br/>SRAM]
> D --> E[输出:B,S,H<br/>写回 HBM]
> ```
> 图中从 Self-Attn 到 FFN 的第 2 箭头对应正文“每个子模块外包残差+Norm”的第 2 步；全程 HBM 驻留 $BSH\times2$B，SRAM 内完成 $QK^\top$ 与 $PV$。
...
- 注意力分数矩阵：$(N_{tgt} \times N_{src})$——不是方阵！  // Shape 守恒：$[N_{tgt},d]\times[d,N_{src}]\to[N_{tgt},N_{src}]$，归约维 $d$ 消失；分数矩阵 $N_{tgt}N_{src}\times2$B 驻 HBM，SRAM 内分块计算 $QK^\top$
...
- ❌ **一上来就钻进 Self-Attention 的矩阵乘法细节**：“只见树木不见森林”。为什么错：注意力只是 Encoder/Decoder 里的一环，没有整体框架时细节记不住也连不起来。正确做法：先用本文的全貌 + 代码实现把 Encoder/Decoder 数据流走通，再回头抠注意力细节。
- ❌ **把 Encoder 和 Decoder 的注意力混为一谈**：以为 Decoder 里的注意力跟 Encoder 完全一样。为什么错：Decoder 有 Masked Self-Attention（因果掩码）和 Cross-Attention（Q 来自 Decoder、K/V 来自 Encoder）两种额外机制。正确做法：分清楚三种注意力各自的输入输出。
- ❌ **只读公式不跑代码**：看完架构图就以为懂了。为什么错：维度错误、广播错误只有跑起来才会暴露。正确做法：用本文的 PyTorch 实现亲手 forward 一遍，跟踪每个张量的 shape。
- ❌ **忽略输入 Embedding 与位置编码**：从“注意力层”开始读。为什么错：输入表示（词嵌入 + 位置）决定了模型能看到什么信息，是推理正确性的起点。正确做法：从 Embedding + Positional Encoding 一路跟到输出。
