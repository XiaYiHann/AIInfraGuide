...
> 💡 **最小走查（$d_{model}=4096,h=32$ 分 4 卡）：** 每卡 $W_Q$ 切为 $(4096,1024)$ 各自本地算 8 头，最终 $W_O$ 前需 1 次 **AllReduce** 汇总，Ring 通信量≈$2(N-1)/N\cdot M$。
...
1. **AI Infra 的本质**：让模型训练与推理“快、稳、省”的工程体系——Transformer 是它的唯一业务对象，所有优化（算子、并行、部署）都在围绕 Transformer 的同一套模块转。
2. **Transformer 为什么是通用底座**：Attention 机制解决了 RNN 无法并行与长距离依赖两大困境，从原始论文到 LLM 家族（Decoder-only 主导）一路收敛为事实标准。
3. **层级对应关系**：CUDA 层优化 Attention/FFN 算子、分布式层切分参数与通信、推理层管理 KV Cache 与调度——同一张架构图，三层各取所需。
4. **端到端旅程**：一次推理请求 = Tokenize → Embedding → Prefill（并行处理整个 Prompt）→ Decode（逐 Token 生成）→ 采样输出，各阶段瓶颈不同。
5. **学习路径建议**：先“见树”（吃透单个模块）再“见林”（看全链路），学每一层时都问一句“它在 Transformer 的哪个环节、优化了什么”。

## ⚠️ 常见错误做法

- ❌ **不读代码、只背概念就开始“优化”**：一上来就研究 CUDA 或并行策略，却连一次完整的前向传播都讲不清。为什么错：AI Infra 的每一层优化都锚定在 Transformer 的具体模块上，没有全链路认知时做的优化是盲目的。正确做法：先用本文的端到端案例把“一次请求从 Tokenize 到输出”走通，再逐层深入。
- ❌ **把“会用 vLLM/DeepSpeed”当成“懂 AI Infra”**：只会敲命令调参数。为什么错：不懂引擎内部的调度与显存账本，出问题只能瞎试。正确做法：每个工具背后问一句“它优化了 Transformer 的哪个环节、牺牲了什么”。
- ❌ **只见树不见林**：钻进 Self-Attention 的矩阵细节，却不知道它在整条链路里扮演什么。为什么错：知识碎片化，无法迁移到新架构。正确做法：先“见林”（全链路）再“见树”（单模块），保持“这个知识点属于哪一层”的意识。
...
- 能说出 AI Infra 的三层职责（算子优化/分布式/推理部署）分别对应 Transformer 的哪些模块，并各举一个具体优化动作
