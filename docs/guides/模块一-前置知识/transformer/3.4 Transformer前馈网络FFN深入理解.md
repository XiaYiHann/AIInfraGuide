...
这套“参数扩 1000 倍但单 token 计算几乎不变”的架构与训练工程四件套（noisy top-k、专家坍缩、shrinking batch、混合并行），源头是 Shazeer 2017 的 Sparsely-Gated MoE 原始论文，见 [3.12 参数扩 1000 倍但计算不变：MoE 的诞生](/AIInfraGuide/prerequisites/模块一-前置知识/transformer/312-moe-sparsely-gated-moe论文精读)。
...
- **Decode 阶段**（每次只处理 1 个 token）：GEMM 退化为矩阵-向量乘法，计算量小，kernel launch 和 HBM 带宽是主要瓶颈，融合收益显著（可达 20-40% 的加速，工程经验估算，待核实）
- **Prefill 阶段**（处理整段 prompt）：GEMM 的计算量占主导，逐元素操作的 HBM 开销占比较小，融合收益相对有限（通常 5-15%，工程经验估算，待核实）

---

## 📝 总结

1. **FFN 的角色**：Transformer 里“记住与变换”的部分——每个 token 独立地先升维再降维（如 4096→11008→4096），约占模型 2/3 的参数。
2. **激活函数演进**：ReLU → GELU → SwiGLU；SwiGLU 用门控机制（两个线性层逐元素相乘）成为大模型主流，同样的参数量下效果更好，中间维度用 8/3 倍扩展补偿。
3. **并行切分**：张量并行下 FFN 的列切分 + 行切分天然适配 Megatron-LM 的两阶段 AllReduce；MoE 则把 FFN 拆成多个专家 + 路由器，配合 Expert Parallelism 摊到多卡。
4. **工程优化**：FFN 的 Kernel 融合（减少 HBM 往返）在 Decode 阶段收益显著（可达 20-40%，工程估算），Prefill 阶段收益有限；中间维度 4x/8/3x 是“表达能力与参数量”的权衡结果。

## ⚠️ 常见错误做法

- ❌ **用 ReLU 直接替换 GELU/SwiGLU 以为没差别**：只看到“都是非线性”。为什么错：SwiGLU 的门控机制给梯度两条通路，效果与训练稳定性都更好；ReLU 还有 Dying ReLU（神经元死亡）问题。正确做法：新模型按主流选 SwiGLU，迁移学习再评估是否换激活。
- ❌ **手算参数量时忘记 FFN 占大头**：只算 Attention 的四矩阵。为什么错：FFN 通常占模型约 2/3 的参数（7B 里约 4.7B 在 FFN），漏算会让显存账本错得离谱。正确做法：参数量手算把 Attention + FFN + Embedding 全算上（见 3.7 的完整公式）。
- ❌ **把中间维度随意改小“省参数”**：4x/8/3x 扩展是有原因的。为什么错：中间维度太小，表达能力不足，模型效果明显下降；这是“参数量-效果”的权衡，不是随便省的。正确做法：按 4x（标准 FFN）或 8/3x（SwiGLU）起步，用实验验证再动。
- ❌ **张量并行切 FFN 时只切一个矩阵**：比如只切 down 不切 gate。为什么错：SwiGLU 的 gate 与 up 必须同样切分才能保持计算等价，切分不一致会导致结果错误。正确做法：按 Megatron-LM 的列切 + 行切两阶段，gate/up 一起切。
