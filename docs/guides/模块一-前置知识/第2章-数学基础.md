...
> 💡 **三账本一瞥（以 §1.2 输出投影为例，$B=8,S=2048,H=4096,V=32000$）**
> | 账本 | 数量级 | 落点 |
> |---|---|---|
> | 计算 | $2BSHV\approx2.1\times10^{12}$ FLOPs | Tensor Core GEMM |
> | 内存 | 输出 $BSV\times2$B$\approx1.05$ GB (FP16) 驻 HBM，SRAM 内累加 | HBM→SRAM |
> | 通信 | 单机 $0$ Byte；张量并行时 AllReduce $\approx2BSH\approx0.13$ GB/层 · NVLink |
> 估算口径：一次前向输出投影约 1 GB 写回 HBM，算术强度 $>200$ FLOP/Byte 属算力受限，与 Decode 的访存受限形成对照。
...
把形状当作接口检查（Shape 守恒：$(M,K)$ 的 $K$ 与 $(K,N)$ 的 $K$ 对消，只留 $(M,N)$；HBM 驻留 $A,B,C$ 三份约 $2(MK+KN+MN)$ Byte，SRAM 内累加器复用 $K$ 次）：

```text
(M, K) @ (K, N) -> (M, N)  // [M,K]×[K,N]→[M,N]，归约维 K 消失，Batch 维保留
```
...
> 💡 **Roofline 一瞥（H800：989 TFLOPS / 3.35 TB/s，拐点≈295 FLOP/Byte）**：取 $M=N=K=4096$，FLOPs$=2\times4096^3\approx1.37\times10^{11}$，HBM 往返约 $2(3\times4096^2)\times2$B$\approx201$ MB，$AI\approx682$ FLOP/Byte ≫295 属**算力受限**；同卡 Decode 访存强度 $<5$ FLOP/Byte 属**访存受限**。图中每条 GEMM 的 $M,K,N$ 决定它落在 Roofline 哪一侧。
