...
- ❌ **以为 Tensor Core 是“通用加速器”，什么算子都能用**：把非矩阵乘的代码也往 MMA 上套。为什么错：Tensor Core 只加速矩阵乘/卷积类的规则计算（4×4 MMA），elementwise、规约、动态形状都不适用。正确做法：先看算子能否表达成 GEMM，再决定是否上 Tensor Core。
- ❌ **FP8 当“免费午餐”**：只看到显存减半、速度翻倍，忽略缩放因子。为什么错：FP8 动态范围小，需要 per-tensor/per-block 缩放（scaling factor），不管理缩放精度会崩。正确做法：按官方 FP8 流程配缩放策略，并做精度验证。
- ❌ **在 Ampere 上硬用 Hopper 的指令**：把 5 代 Tensor Core 的特性混为一谈。为什么错：WGMMA、FP8 原生支持等是 Hopper+ 特性，Ampere 只支持 3 代 MMA。正确做法：按硬件代际选指令与精度。
- ❌ **只看峰值算力不看数据搬运**：以为 MMA 快整个 Kernel 就快。为什么错：数据喂不进 Tensor Core（内存带宽/Shared Memory 瓶颈），算力再高也闲置。正确做法：用 Roofline 确认是计算瓶颈还是访存瓶颈。
