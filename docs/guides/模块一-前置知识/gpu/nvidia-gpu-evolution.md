...
- ❌ **把“新一代 GPU”简单当成“更快的老 GPU”**：用老架构的经验直接套新卡。为什么错：每一代都在改架构重点——Volta 引入 Tensor Core、Ampere 强化稀疏与 MIG、Hopper 加 TMA/集群、Blackwell 强化 FP8/FP4——优化手段要跟着换代。正确做法：先读该代的白皮书，确认新增硬件特性再优化。
- ❌ **忽略显存带宽的换代速度**：只看算力翻倍就预期性能翻倍。为什么错：算力增速远快于带宽增速（Memory Wall），新卡上 Memory-Bound 算子提升有限。正确做法：用算力与带宽的比值（算术强度平衡点）判断瓶颈。
- ❌ **不区分 PCIe 版与 SXM 版**：买卡/写代码时混用。为什么错：SXM 版带宽与功耗更高、支持 NVSwitch 多卡互联，PCIe 版便宜但互联弱；多卡拓扑直接影响并行策略选择。正确做法：部署前确认卡型与互联拓扑（`nvidia-smi topo -m`）。
- ❌ **只追单卡指标，忽略集群视角**：选卡只看单卡 TFLOPS。为什么错：大模型训练/推理是集群行为，NVLink/InfiniBand 拓扑决定通信成本（见 5.7）。正确做法：单卡 + 集群网络一起评估。
