...
拒绝发生后，位置 $i$ 必须输出一个 token。有三个候选做法：

1. **回退到上一个已接受的 token，重新来**——❌ 错。这样输出序列会偏向"短且重复"，且当前位置没有产出，浪费了这轮验证。
2. **无条件从 target 分布 $p$ 重采样**——❌ 也错。直觉上"从 p 采样总没错"，但数学上它**有偏**。为什么？因为"被拒绝"这个事件本身携带信息：它说明 draft 在 $q$ 高估的区域抽中了样本，此时无条件按 $p$ 采样，等于把 $p$ 里那些被 $q$ 抢走概率质量的位置又补了一份。看一个 2-token 词汇表的数值反例：$q(a)=0.9, q(b)=0.1$，$p(a)=0.6, p(b)=0.4$。接受概率 $a(a)=0.6/0.9=2/3$、$a(b)=1$，整体接受率 $\alpha = 0.9 \times \frac{2}{3} + 0.1 \times 1 = 0.7$。如果拒绝时从 $p$ 无条件采样，输出 $a$ 的概率 $= q(a)a(a) + 0.3 \times p(a) = 0.6 + 0.18 = 0.78 \neq 0.6$——**偏了**。
3. **从修正分布重采样**——✅ 对。直觉：拒绝后要补上的，正是大模型比小模型更看好的那部分概率。从
$$
r(x) = \frac{\max(p(x) - q(x),\ 0)}{1 - \alpha}
$$
采样，其中 $\alpha = \sum_{y \in \mathcal{V}} q(y) a(y)$ 是整体接受率，$1-\alpha$ 是归一化常数。**这个分布专门负责"补上 $p$ 里被 $q$ 低估的部分"**——$p \leq q$ 的位置概率为 0，$p > q$ 的位置按差额比例分配。

#### 为什么“从 $p$ 重采样”会偏：$p$ 不是拒绝分支的条件分布

这里最容易混淆的一点是：**最终输出的边际分布要是 $p$，不代表拒绝分支里的条件分布也要是 $p$**。拒绝前已经发生了一次带筛选的采样，接受路径已经贡献了一部分概率质量；拒绝分支只负责把剩余部分补齐，而不是重新生成一份完整的 $p$。

设 Draft 先采样 $Y\sim q$，$A$ 表示接受，$R$ 表示拒绝。对任意 token $x$，接受路径“接受并输出 $x$”的**联合概率**是

$$
\Pr(A, Y=x)=q(x)a(x)=\min\bigl(q(x),p(x)\bigr)
$$

因此，接受路径总共已经贡献了

$$
\alpha=\Pr(A)=\sum_x \min\bigl(q(x),p(x)\bigr)
$$

的概率质量。若拒绝后无条件地从 $p$ 采样，即令 $\Pr(Z=x\mid R)=p(x)$，最终输出概率会变成

$$
\Pr(X=x)
=\underbrace{\min\bigl(q(x),p(x)\bigr)}_{\text{接受路径已经提供}}
+\underbrace{(1-\alpha)p(x)}_{\text{拒绝路径又提供一整份 }p(x)}
$$

这通常不等于 $p(x)$：有些 token 在接受路径中已经达到 target 所要求的概率，拒绝路径再按完整的 $p$ 补一次，就会重复计算。

正确的做法是先算“接受路径之后还缺多少”。对每个 token，target 还需要的概率质量为

$$
\begin{aligned}
p(x)-\min\bigl(q(x),p(x)\bigr)
&=\max\bigl(p(x)-q(x),0\bigr)\\
&=\text{target 比 draft 多看好的那部分概率}
\end{aligned}
$$
...
$$
\sum_x\max\bigl(p(x)-q(x),0\bigr)
=1-\sum_x\min\bigl(q(x),p(x)\bigr)
=1-\alpha
$$

所以，拒绝分支的条件分布必须是“剩余质量除以拒绝总量”：

$$
\Pr(Z=x\mid R)=r(x)
=\frac{\max\bigl(p(x)-q(x),0\bigr)}{1-\alpha}
$$

仍用上面的二 token 例子看这笔账：

| token | $q(x)$ | $p(x)$ | 接受路径质量 $q(x)a(x)$ | target 还缺的质量 |
|---|---:|---:|---:|---:|
| $a$ | $0.9$ | $0.6$ | $0.6$ | $0$ |
| $b$ | $0.1$ | $0.4$ | $0.1$ | $0.3$ |
| **合计** | **1.0** | **1.0** | **$\alpha=0.7$** | **$1-\alpha=0.3$** |

所以拒绝发生时，正确的修正分布是 $r=(0,1)$：这次拒绝分支必须输出 $b$。如果错误地从 $p=(0.6,0.4)$ 采样，就会额外贡献 $(0.18,0.12)$，最终得到 $(0.78,0.22)$，而不是 $(0.6,0.4)$。

“拒绝事件携带信息”也可以严格写出来：

$$
\Pr(Y=x,R)=q(x)\bigl(1-a(x)\bigr)=\max\bigl(q(x)-p(x),0\bigr)
$$

也就是说，拒绝只会发生在 $q$ 高估的区域；但为了把最终分布修正回 $p$，接下来要补的是 $p$ 高估、而 $q$ 低估的区域。**接受分支提供两者的重叠部分，拒绝分支提供 target 独有的剩余部分；直接从完整的 $p$ 采样会把重叠部分再次加进去。**
