...
先回答一个最朴素的问题：模型在开发机跑得好好的，为什么到了生产就“水土不服”？因为推理服务对环境极其挑剔。驱动与 CUDA 版本要互相匹配（好在**驱动向后兼容**——新驱动能跑旧 CUDA 编译的镜像，反过来不行）；还要装齐 torch/transformers 等几十个 Python 包，以及 libnccl/libcublas 等系统二进制库。任何一层对不上，轻则起不来，重则静默算错。
...
各自的代价：挂载方式**换取了**“换模型不用重新构建镜像”,但**牺牲了**“镜像到哪都能跑”（依赖宿主机文件）；打进镜像**换取了**部署确定性，但**牺牲了**灵活性——而且 7B 模型 FP16 权重约 14GB,镜像动辄几十 GB,推送和拉取都慢。**选型规则：多模型、常换模型 → 挂载；版本强管控、离线交付 → 打进镜像。**
...
这里有一条硬规则必须刻进脑子：**GPU 是“不可压缩资源”，不能超卖**。CPU 是可压缩资源——多个 Pod 可以共享一个核，忙时大家排队，慢一点而已；GPU 不行，一个 Pod 占住显存和算力，另一个 Pod 挤进来只会双双 OOM（内存耗尽被系统杀掉）。所以 K8s 规定：
...
**模型加载有多慢？** vLLM 启动要做四件事：下载/读取权重 → 加载进显存 → 为不同 batch size 捕获 CUDA Graph → 跑一遍 profiling 确定 KV Cache 分配。社区经验值（无一手出处，待核实）：**7B 模型暖缓存（权重已在本机）要 60~120 秒；70B 模型从 Hugging Face 下载要 15~30 分钟**。
...
Router 的实现思路（官方源码语义，细节随版本演进）：把 prompt 切成 **128 字符的块，每块算一个哈希值（xxHash64，一种快速指纹算法）**；内存里用一棵哈希前缀树（HashTrie）记住“这个前缀上次由哪个后端服务过”。新请求按哈希块走这棵树，**最长前缀匹配**到后端，让 vLLM 的 Prefix Cache 直接复用。冷启动没命中，就发给当前 QPS 最低的实例，并记录前缀，让后续相似请求“粘”上来。更精确的 `kvaware` 变体则查 LMCache 的缓存布局再路由（官方路线图标注 prefix-aware 仍在演进，以你所用版本的文档为准）。
...
KServe 是 K8s 生态里最成熟的模型服务框架，定位是**模型的控制面**：用 CRD（自定义资源，K8s 用来描述“模型”这类新对象的机制）声明“这个模型怎么跑、几个副本、从哪拉权重”，生命周期（部署、伸缩、更新）交给控制器。核心抽象：
...
- ❌ **GPU 服务不声明 `resources.limits.nvidia.com/gpu`**：在 K8s 里裸跑推理容器。为什么错：没有资源声明，调度器可能把多个 GPU 任务塞进同一张卡或根本不分配 GPU，nvidia-device-plugin 无法生效。正确做法：显式声明 `limits: nvidia.com/gpu: 1`，且不要给 GPU 服务配 CPU 超卖。
- ❌ **探针阈值小于模型加载时间**：startupProbe 默认 30 秒，70B 加载要 15-30 分钟。为什么错：Pod 被反复误杀重启（CrashLoopBackOff），一晚上服务还在转圈。正确做法：startupProbe 给足窗口（1.5-2 倍实测启动时间），liveness/readiness 用紧凑阈值（10.1 的配置表）。
- ❌ **镜像标签用 `latest`**：部署“最新”镜像。为什么错：`latest` 随发布漂移，今天部署的和下个月部署的可能不是同一个行为，回滚困难。正确做法：用不可变标签（commit SHA 或版本号）。
- ❌ **权重每次启动现下**：Pod 启动时从 HuggingFace 拉权重。为什么错：70B 冷下载 15-30 分钟（社区经验值），每次重启都是灾难。正确做法：权重挂载到 PVC/对象存储，镜像只含代码（10.1 的挂载方案）。
