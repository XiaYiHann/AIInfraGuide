...
- ❌ **用 CPU/内存做 GPU 服务的扩缩容主指标**：CPU 利用率低就缩容。为什么错：GPU 服务排队时 CPU 可能只有 20%（在等 GPU），按 CPU 缩容会直接砍掉正在排队的容量。正确做法：用 GPU 相关指标（`vllm:num_requests_waiting`、GPU 利用率）做主指标。
- ❌ **把 HPA 的 min/max 与容量规划的采购量混为一谈**：max 填采购总量。为什么错：采购量含冗余（本文 10.4 的 180 副本），HPA max 是峰值需求（125 副本），填错会让 HPA 把容量撑到冗余线，成本失控。正确做法：max = 峰值需求副本数，采购量是节点池预算。
- ❌ **负载均衡按会话粘性做哈希**：把同一用户的请求固定到同一副本。为什么错：GPU 推理无状态（KV 在显存内），粘性哈希会导致热点副本排队。正确做法：按请求长度/优先级路由，前缀路由只用于 Prompt Cache 命中。
- ❌ **忽略缩容的冷却时间**：流量一降立刻缩容。为什么错：K8s HPA 默认缩容冷却 5 分钟，流量抖动会被放大成“扩容-缩容”震荡。正确做法：配置 `--horizontal-pod-autoscaler-downscale-stabilization-window` 与扩容/缩容系数，按 10.3 的调参表设置。
