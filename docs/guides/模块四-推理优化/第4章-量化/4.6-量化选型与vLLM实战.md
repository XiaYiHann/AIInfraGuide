...
先给一个生活类比：选型就像**搬家装箱**——**先看箱子够不够大**（显存装不装得下权重和 KV），**再看搬运速度**（带宽决定每步快慢），**最后才决定要不要拆家具**（牺牲精度）。拆得越碎（位数越低），箱子越省，但家具越容易损坏（精度风险越高）。后面所有决策树、账本表，都是在帮你回答这三个问题。
...
- **选型是问诊**：先确认症状（权重放不下 / KV 放不下 / Prefill 卡 / Decode 卡），再定位硬件与精度余量，最后开药——决策树五问 + 决策规则。
...
- ❌ **不诊断瓶颈就选量化方案**：听说 INT4 省显存就全上 INT4。为什么错：显存紧张、带宽紧张、算力紧张对应的方案完全不同（INT4 / W8A8 / 不量化）。正确做法：按 4.6 的决策树先问“瓶颈是显存、带宽还是算力”，再选方案。
- ❌ **vLLM 里开了量化但没开对应 kernel**：加载了 AWQ 权重却用默认后端。为什么错：量化权重需要对应的反量化 kernel 支持，配置不对会报错或静默退化。正确做法：`--quantization awq/gptq` 与权重格式匹配，用 `vllm serve --help` 确认参数名（版本差异大）。
- ❌ **精度评测只跑困惑度**：PPL 不涨就宣布无损。为什么错：困惑度对量化不敏感，任务指标（推理/代码/事实性）才反映真实损失。正确做法：用目标任务 benchmark 对比 FP16 vs 量化（4.6 的评测协议）。
- ❌ **量化版本与生产版本不一致**：压测用 A100 + 一种量化，上线用另一种卡。为什么错：不同硬件对量化的支持与性能差异巨大。正确做法：压测环境 = 生产环境（卡型、量化、batch 完全一致）。
