...
- ❌ **在 Ampere 上硬上 FP8**：把 Hopper 的格式搬到旧卡。为什么错：FP8 的 Tensor Core 支持从 Hopper 开始，Ampere 没有原生 FP8 指令，硬跑会退化成模拟，反而更慢。正确做法：按硬件选精度（Ampere 用 INT8/FP16，Hopper+ 才考虑 FP8，Blackwell 看 NVFP4）。
- ❌ **FP8 训练不做缩放管理**：以为 FP8 是“免费的 BF16”。为什么错：FP8 动态范围小，需要 per-tensor/per-block 缩放（scaling factor）与溢出检测，不管理训练会发散。正确做法：按官方 FP8 训练流程（如 DeepSeek 的 FP8 方案）配缩放，并监控 inf/nan。
- ❌ **把 NVFP4 当成“更小的 FP8”直接替换**：4bit 无脑上。为什么错：NVFP4 的精度损失更大，需要更细的缩放粒度与算法配合（如 CU 分块），不是所有模型都扛得住。正确做法：先验证精度（4.5 的评测流程），再决定。
- ❌ **忽略混合精度层**：全模型统一 FP8。为什么错：敏感层（首尾层、输出层）FP8 误差更大。正确做法：关键层保留更高精度（混合策略，4.5 有对照）。
