...
升级路径：**先开 `kv_cache_dtype="fp8"` 白捡 2 倍；还不够，再上 KIVI 拿 4 倍**——绝大多数场景下 FP8 已经够用，KIVI 是留给“显存真的不够”的场景的。
...
决策规则：**短上下文不量化 KV，中等需求开 FP8，显存真不够才上 2-bit；量化永远以“省下的显存换成了更大 batch”为唯一验收标准**。
