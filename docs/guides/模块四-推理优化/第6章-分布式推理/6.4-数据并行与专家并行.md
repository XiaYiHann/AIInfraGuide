...
训练里的数据并行每步都要 AllReduce 对齐梯度，推理里的数据并行**什么都不用传**——稠密模型的多副本推理是零通信的，开 N 个副本，在负载均衡理想的线性区就把吞吐放大 N 倍。但 MoE 模型把账本彻底改写了。DeepSeek-V3 共 671B 参数、每 token 只激活 37B,FP8 权重整整 671GB,8 张 H100（640GB））都装不下完整权重（8×H200 才行），必须把 256 个专家摊到各卡（专家并行 EP）。代价是每个 token 都要经历两次 All-to-All 搬家：batch 256 时每层搬 29MB、58 个 MoE 层共约 1.7GB，通信量是同样 batch 下 TP 的 $K/2 \approx 4$ 倍。本文把这两张牌讲透：DP 怎么零成本扩吞吐，EP 为什么是千亿 MoE 推理的必选项，以及 vLLM 怎么把 All-to-All 藏进计算里。
...
数据并行（DP）的思路完全不同：**不切模型，复制模型**。每块卡（或每组卡）跑一份完整引擎，各接各的请求，互不相干。打个比方：**一家店客流爆满，你不改厨房流程，而是直接开分店——每家分店菜单完全一样（同样的权重副本），各自独立接客、独立排队，店与店之间不需要打电话对账**。用技术语言说：推理的 DP 副本之间**没有任何梯度需要同步**，权重是只读的，所以副本之间零通信。

对比训练 DDP（模块三 4.1）：训练每步都要 AllReduce 梯度把副本"对齐评分尺度"；推理权重冻结，没有这个动作。**训练 DP 的通信量是 $2\Psi$（每步全量梯度），推理 DP 的通信量是 0**——这是推理侧最便宜的一种并行。
...
- **形态二：内置 DP**。`vllm serve --data-parallel-size 4` 把 4 个"core engine"进程拉在同一个 API server 后面（进程间走 ZMQ），由内部负载均衡按各引擎的 running/waiting 队列分发请求。它和 `--tensor-parallel-size` 可组合：`--data-parallel-size 4 --tensor-parallel-size 2` 需要 8 卡——4 个副本，每个副本内部 TP2。
...
DeepSeek-V3 是当前 MoE 部署的标准样本（技术报告原文："671B total parameters with 37B activated for each token"）。先算清它的结构（配置来自官方 config.json，数字为近似值）：
...
TP 切的是**单个权重矩阵**（每个专家都被切成 N 片）；EP 切的是**专家本身**——256 个专家按路由热度分配到 N 张卡，每卡只驻留 $256/N$ 个专家的完整权重（模块三第 10 章的结论直接复用）。
...
- **dispatch**：把 token 的 hidden state（h 维向量）发给它选中的 8 个专家所在的卡——同一专家收到的多个 token 打包成一个 batch 一次算完（专家侧的大 batch 是 EP 算力效率的来源）。
- **combine**：各专家卡把 FFN 结果发回原卡，原卡按 router 权重加权求和。
...
📌 **关键点**：同一套 A2A，prefill 和 decode 是两种病——这正是 vLLM 提供两个 DeepEP 后端、以及第 7 章 PD 解耦把两个阶段拆到不同实例的原因。
...
- **DP attention 的意义**：6.2 第 3.3 节留的伏笔——MLA 模型在 TP 下 KV 相关结构在 shard 间复制、浪费显存（vLLM 官方 Issue #16037 原话："If TP is used in an MLA model, we duplicate the KV cache across GPUs"）。DP attention 让每个副本只持有自己那批请求的 KV，**副本间按请求切 KV，而不是按层切**——这是 DeepSeek-V3/R1 推荐 DP+EP 而不是纯 TP 的核心原因。
