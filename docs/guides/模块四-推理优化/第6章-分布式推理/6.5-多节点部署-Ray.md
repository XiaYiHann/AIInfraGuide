...
先给结论：**单机多卡的 vLLM 默认根本不用 Ray**——官方默认的分布式运行时是 Python 多进程（multiprocessing），Ray 只有在**跨节点**部署时才出场，而且它只负责"编排"，不碰任何计算。多节点部署 80% 的故障发生在 NCCL 这一层（卡在初始化、IB 不识别、超时），而不是模型层——这些坑大多可以通过两条环境变量和一行日志检查提前规避。本文承接 6.2/6.3 讲完的 TP 与 PP，解决"怎么把并行度落到两台以上的机器上"：Ray 基础概念（head/worker、Placement Group、资源 bundle）、部署三步走、排错表、拓扑检查，以及"**单机装得下就别上 Ray**"的决策规则。
...
那为什么还有这么多人把"vLLM 多卡"和"Ray"绑定在一起？因为**模型一旦大到单机装不下，vLLM 自己搞不定"把进程放到另一台机器上"这件事**。多进程模式假设所有进程在同一台机器的同一个文件系统里（fork/spawn 都是本地操作）；跨机器时，谁来决定哪个进程跑在哪台机器、哪张卡上？这需要一个跨节点的"调度中枢"——Ray。

### 1.2 Ray 的角色：调度台，不是计算者

打个比方：把模型部署比作搬家公司搬一套大件家具。单机多卡是你一个人在一间仓库里调配几台叉车（GPU），自己喊就行；跨节点相当于家具大到一间仓库放不下，要**两个仓库协同搬运**——这时候你需要一个调度台（Ray）：它登记每个仓库有几台叉车、派单决定"哪台叉车去哪个仓库、什么时候动"，但它**不负责开叉车**。开叉车（GPU 之间传数据）是 NCCL 干的，调度台管不到也不该管。
...
什么时候跨节点？显存账本说了算（6.1 的口径）：70B FP16 权重 140GB，单机 8×A100 80GB 勉强装下但 KV 余量吃紧；**Llama-3.1-405B FP16 权重 810GB，8×80GB = 640GB 连权重都放不下**——官方文档的多节点示例正是拿 405B 演示：2 节点各 8 卡，`--tensor-parallel-size 8 --pipeline-parallel-size 2`。跨节点那一刻起，你就要和 Ray、NCCL 环境变量、IB 网卡打交道了。
...
- **head 节点**：调度中枢。`ray start --head --port 6379` 启动，负责资源登记、任务派发、元数据存储（GCS）。一个集群只有一个 head。
...
💡 **提示**：PACK 是默认策略，vLLM 靠"每节点 8 卡、TP=8"的整数关系自然做到 TP 机内、PP 跨机。**官方推荐的多节点配置就是 TP = 每节点 GPU 数、PP = 节点数**——这个"推荐"不是文档拍脑袋，而是让 PACK 策略下的 bundle 恰好按节点对齐。如果每节点卡数不均匀（比如一台 8 卡一台 4 卡），PACK 会把 12 个 bundle 里的 8 个塞进第一台，TP 组照样跨机——这种拓扑要你自己想清楚再配。
...
- **NODE**：跨 NUMA 节点，带宽比同 NODE 差一截；
- **PIX / PHB**：PCIe 直连或经 PCIe 桥，比 NVLink 慢一个数量级。
...
💡 **提示**：8 卡整机通常还有 NVSwitch 全互联（任意两卡直连，topo 里表现为满 NVLink），此时 TP8 的任意 rank 对之间都是最高速路径；而 SYS 标记出现在 TP 组内部，说明你的 TP 组被跨节点/NUMA 拆开了，通信要绕系统总线——**配置没问题但性能不对时，先看 topo 是不是 SYS 混进了 TP 组**。
...
- **网络假设**：跨节点必须依赖 IB/RoCE 质量，TCP 回退时性能断崖（3.4 节的验证方法就是为它准备的）。
