...
设 batch 内当前有 $b$ 个 token，hidden 维 $h$，每元素 $d$ 字节（FP16 为 2B），则单次 AllReduce 的传输量为：
...
💡 **提示**：那个"2"不是"两次 AllReduce"，而是 **AllReduce 自身的通信冗余**——2.1 集合通信讲过，Ring AllReduce 的通信量 ≈ $2(N-1)/N \times$ 数据量，卡数多了就趋近 2 倍（每卡各发一份、各收一份）。模块三第 6 章"每次传输 2bsh"的口径与此一致。
