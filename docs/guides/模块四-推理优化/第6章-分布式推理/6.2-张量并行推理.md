...
**先回答一个三十秒复习题：TP 到底把什么切开了？** 先用一个 2×2 的最小矩阵把“两种切法哪个要通信”算明白，再回到 Transformer。Transformer 每层有两个“必经之路”，先认三个词：**QKV**（注意力里的三组向量：Q 查询、K 键、V 值）、**FFN**（前馈网络：两个线性层夹一个激活函数）、**head**（注意力头：注意力计算的一个独立通道，多头就是多条通道并行）。

💡 **回顾：AllReduce 是什么**：把各卡算出的部分结果相加，最后每张卡都拿到同一份完整结果（Ring 实现通信量 ≈ 2(N-1)/N × 数据量，卡一多就接近 2×）。TP 的通信量几乎全由它构成；详见模块三 2.1 集合通信原语。

**最小走查：为什么“行切要 AllReduce、列切不用”？** 设某线性层 $y = W x$，输入 $x = [1,\ 2]$，权重 $W = \begin{bmatrix}1&2\\3&4\end{bmatrix}$，完整结果是 $y = \begin{bmatrix}1\cdot1+2\cdot2\\ 3\cdot1+4\cdot2\end{bmatrix} = \begin{bmatrix}5\\11\end{bmatrix}$。现在把计算分到两张卡：

- **行切（Row Parallel）——每个输出都被拆成部分和，必须 AllReduce**：卡 1 拿“对 $x_1$ 的贡献” $[1\cdot1,\ 3\cdot1] = [1,\ 3]$，卡 2 拿“对 $x_2$ 的贡献” $[2\cdot2,\ 4\cdot2] = [4,\ 8]$。此时 $y_1 = 1+4$、$y_2 = 3+8$ 各只凑到一半，**任何一张卡自己都拼不出完整的 $y$**——必须在出口 AllReduce 相加（$[1+4,\ 3+8] = [5,\ 11]$）才能得到下一层要的完整向量。Attention 输出投影、FFN 第二个线性层正是这种：权重按**行**切，每卡算部分和，最后必须 AllReduce——因为下一层的输入要求“完整”。
- **列切（Column Parallel）——按输出维拆开，各算各的，无需通信**：卡 1 拿 $W$ 第 1 行 $[1,\ 2]$ 负责算 $y_1 = 5$，卡 2 拿第 2 行 $[3,\ 4]$ 负责算 $y_2 = 11$。两卡各算各的 $y_i$，拼在一起就是完整 $y$，谁也不等谁。QKV 投影、FFN 第一个线性层正是这种：权重按**列**切，每卡只算输出的一部分（多头天然可切），不需要通信——但前一步的输入得是完整的。
