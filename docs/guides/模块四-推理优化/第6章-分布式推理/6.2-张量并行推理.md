...
张量并行（Tensor Parallelism, TP）是推理并行的"第一板斧"：单机多卡、追求单请求延迟，它是默认答案。它的全部代价浓缩在一句话里——**每生成一个 token，每层要做 2 次 AllReduce**。7B 模型（hidden=4096、FP16）一个 step 通信约 1MB:这 1MB 在 NVLink（900GB/s））上只要约 1µs，在机间 InfiniBand（约 50GB/s）上却要 20µs。再叠加 64 次小消息的固定延迟，这就是"TP 不跨节点"的物理根源。本文承接模块三第 6 章的切分原理，从推理视角把 TP 的通信量、KV 切分、vLLM 实现和权衡得失算给你看。
...
模块三第 6 章讲过完整推导，这里只留骨架。Transformer 每层有两个"必经之路"：
...
于是每个 Transformer Block 恰好有 **2 次 AllReduce**（Attention 输出 + FFN 输出），这就是 TP 全部通信的骨架。vLLM 官方文档确认：其分布式实现采用的就是 Megatron-LM 的张量并行算法。
...
### 1.2 推理视角：prefill 和 decode 的通信形态完全不同

训练里 TP 的通信发生在"大块时间"里，可以跟计算重叠；推理要拆成两个阶段看：
...
模块三第 6 章的推导直接复用，但推理场景有三处"不需要"和一处"更严重"：
...
### 2.1 公式：单 token 每层每次 AllReduce

直觉先说清那个系数 2:AllReduce 要把所有卡的部分和加到一起，每张卡既要发一份数据、又要收一份数据，传输量天然翻倍。设 batch 内当前有 $b$ 个 token,hidden 维 $h$，每元素 $d$ 字节（FP16 为 2B），则单次 AllReduce 的传输量为：
...
💡 **提示**：那个"2"不是"两次 AllReduce"，而是 **AllReduce 自身的通信冗余**——2.1 集合通信讲过，Ring AllReduce 的通信量 ≈ $2(N-1)/N \times$ 数据量，卡数多了就趋近 2 倍。模块三第 6 章"每次传输 2bsh"的口径与此一致。

### 2.2 数值算例：7B 模型一个 step 通信 1MB

代入 7B 模型（$h = 4096$，FP16，单 token）：
...
**延迟账**：64 次（32 层 × 2）小消息 AllReduce，每次都有微秒级的固定启动延迟（NCCL 小消息实测约 5µs 量级，待核实，以你机器实测为准）→ 每 step 固定 **~0.3ms**，与 batch 无关。
...
把上一节的账换成"TP8 跨 2 个节点（每节点 4 卡）"：带宽从 900GB/s 掉到约 50GB/s，且每次 AllReduce 都要穿一次机间网络，固定延迟从约 5µs 涨到 10µs+：
...
TP 把 attention head 分到各卡（QKV 列切），每卡只负责自己那部分 head 的 KV，所以 **KV Cache 总量也摊薄到每卡 1/TP**。沿用学习路线 3.1 的 7B KV 账本公式（按路线口径写作 32GB，精确值为 32GiB ≈ 34.4GB 十进制）继续算：
...
打个比方：**KV 切分就像小组作业里各人记各人的笔记——四个人分工各记一部分内容（每卡 1/TP 的 head），不需要互相抄对方的笔记，但最后讨论时每个人只懂自己那块。** 用术语说：PagedAttention（2.1 站内）管理的是"每卡本地的 KV 页表"，TP 下每卡只为自己那 1/TP 的 head 分配和管理 block——两套机制正交，vLLM 在 TP>1 时自动按层和 head 切分 KV 并各自维护 block table，**不需要额外配置**。
...
### 3.3 ⚠️ 例外：MLA 模型的 KV 复制

DeepSeek 系模型用 MLA（Multi-head Latent Attention），KV 被压缩成低秩潜变量，在 vLLM 的 TP 实现里**不能按 head 直接切**——结果就是 KV Cache 在每张 TP 卡上各存一份完整的，TP=8 时 KV 存 8 份。AMD 的 vLLM MoE Playbook 实测口径：这是 TP 在 MLA 模型上的"8× 浪费"，而 **DP 能把 KV 按副本切到 1/8**。这就是 6.1 提到的：DeepSeek-V3/R1 官方推荐 DP+EP 而不是纯 TP 的原因之一（6.4 详解）。
...
TP 的权重切分有固定套路，Megatron-LM 的命名直接写在代码里：
...
GQA/MQA 模型（KV head 远少于 Q head，如 Llama-3.1-70B 只有 8 个 KV head）在 TP 下有个细节：KV head 数必须能整除 TP，否则 vLLM 会**复制 KV head 凑整**（模块三第 6 章讲过同一策略）——这会让"每卡 1/TP 的 KV"在 GQA 下略不精确（复制的 head 会在多卡重复），但数量级不变。选 TP 前先看模型 config 的 `num_key_value_heads` 能不能整除你的 TP。

### 4.4 与量化的组合：TP 切的是"更小的权重"

TP 和量化不是二选一，而是乘法关系——先量化把权重变小，再 TP 把变小的权重摊开（4.6 的 2×A100 跑 70B 案例就是 FP16 装不下 → W8A8 70GB → TP2 每卡 35GB）。组合规则：
