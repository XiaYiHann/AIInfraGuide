...
流水线并行（Pipeline Parallelism，PP）在训练里靠反向传播把流水线双向喂满，推理里没有反向传播（backward），账要重算——而且结论反直觉：**PP 不降低单请求延迟**。直觉是这样：PP 按层把模型切成 $P$ 段，每段放在一个 stage（流水线段，一组 GPU）上，一个请求要依次穿过全部 $P$ 段；PP=2 时每段算半层，两步加起来还是全模型的时间，一分不省。它换来的只有“装得下”和并发吞吐，而且吞吐还要看并发请求数给不给面子：并发 4、PP=4 时流水线效率只有约 57%——剩下 43% 的时间 GPU 在空等，这就是 bubble（流水线气泡）。所以推理侧 PP 的定位很窄：**它是 TP 填满单机后还装不下时的跨节点手段，不是提速手段**。本文把推理场景下 PP 的 bubble、prefill/decode 差异、TP×PP 组合、vLLM 支持现状和决策边界算给你看。
...
**设问：训练章的 PP 骨架是什么？** **PP 按层把模型切成 $P$ 段，每段放在一个 stage（一组 GPU）上，数据以 micro-batch（微批次：流水线里传输的最小数据块）为单位在 stage 间依次流动**。打个比方：**这是一场接力赛——激活值是接力棒，每个 stage 是一个队员，只有拿到棒才能跑自己的那段（算自己那几层），交棒就是 stage 间的通信。如果全场只有一根棒（单请求 M=1），其他队员全程干等——这就是 bubble（流水线气泡：GPU 空等的时间）**。把并发请求切成 $M$ 个 micro-batch 排队进场，就是填气泡的办法——气泡占比用下面公式算。
...
- $M \to \infty$ 时气泡趋于 0，但训练里 $M$ 太大激活显存爆炸，所以用 1F1B（一个前向接一个反向的调度方式）把峰值激活从 $M$ 份压到 $P$ 份

代几个数值：$P=2$ 时，$M=1$ 气泡占比 $(2-1)/(1+2-1)=1/2$（一半时间空等）；$M=4$ 时降到 $(2-1)/(4+2-1)=1/5$——micro-batch 越多气泡越小，这就是推理场景必须把并发请求合并成足够大的 batch 再塞进流水线的原因。

想从训练侧看完整推导（GPipe 的 filling/draining 步、1F1B 为何能把峰值激活从 $M$ 份压到 $P$ 份），见[模块三 7 章 流水线并行](/AIInfraGuide/distributed/模块三-分布式训练/第7章-流水线并行)——该章从训练视角推导同一公式；本文只复述推理选型需要的部分。
...
### 2.1 差异一：没有反向传播（backward），流水线简化了什么

**设问：少了一个方向，流水线变简单了吗？** 是的，但也少了一张底牌。训练里 PP 最复杂的部分——1F1B 的 warmup/steady/cooldown 三段调度、backward 与 forward 的交叉、激活释放时机——全部源于**反向传播依赖前向**。推理只有前向，流水线退化成最简单的“生产节拍”：每个 stage 拿到上一段传来的激活，算完传给下一段，没有回流。
...
### 2.2 差异二：micro-batch 的个数为什么不由你定

**设问：训练里 M 是参数，推理里为什么不行？** 训练里 $M$ 是你调的参数，想让 bubble 小就加大 $M$（代价是显存）；推理里流水线里同时在飞的 micro-batch 数 ≈ **并发请求数**（vLLM V1 的实现里，一个 decode 请求同一时刻只能在一个 in-flight microbatch 中，官方 RFC #11945 原文如此）。所以训练章的 bubble 公式在推理 decode 稳态下变成：
...
**设问：没有 backward 之后，各 stage 的显存压力一样大吗？** 不一样。训练里 PP 各 stage 的显存压力主要由激活的 1F1B 动态决定，大体均衡；推理里激活不重要了，**权重成为显存主角，而权重在层间天然不均衡**：
...
代入数字感受一下（思想实验，数值为估算）：假设单卡 prefill 一个 4K token 的 prompt 要 1.6s，PP=4 理想均匀切分下每 stage 只要 0.4s——于是 TTFT ≈ 4 × 0.4s + 3 次传输 + 调度 ≈ **1.6s + ε**。第一项恰好等于单卡 prefill 时间，加上的全是纯损耗：**PP 不降低 prefill 延迟，只是把“一块大计算”换成“P 块小计算 + P-1 次传输”**。传输本身不贵（下一节算），贵的是：
...
### 3.2 好消息：prefill 的跨段传输贵吗

**设问：多传的那 P-1 次，会不会很贵？** 不贵——prefill 的传输是大块消息，带宽吃得饱。传输量公式（训练章第 5 节口径：Stage 间传输 = $b \times s \times h$）：
...
### 4.2 代价一：为什么算力翻倍，单请求延迟一分不省

**设问：多一倍 GPU，单请求怎么不加速？** 因为两个 stage 是串行接力而不是并行各算各的。单请求 decode 每步的延迟是**串行穿过整条流水线**的总和：

$$
T_{\text{step}} = \underbrace{P \times t_{\text{stage}}}_{\text{串行经过 P 个 stage}} + \underbrace{(P-1) \times t_{\text{xfer}}}_{\text{每步 P-1 次传输}}
$$

代入 70B、TP8+PP2（每节点 8 卡，每 stage 背 8.75GB 权重，H100 约 3.35TB/s 带宽下限估算，数字待实测），分三步算：

1. 先算单机 TP8 全模型：每 step 每卡要读 17.5GB 权重 → 17.5GB / 3.35TB/s ≈ **5.2ms**；
2. 再算 PP2 下每个 stage：每 stage 只有 8.75GB → 8.75 / 3.35 ≈ **2.6ms**；
3. 最后加总：单请求要串行穿过 2 个 stage → 2.6ms + 2.6ms + 跨机传输 ≈ **5.2ms + ε**。
...
### 4.3 代价二：decode 的跨段传输有多贵

**设问：每步都要传一次的小消息，贵吗？** 单看带宽不贵，贵在固定延迟。decode 的 stage 间传输是**小消息**：单 token（$b=1, s=1$）每边界只有 $h \times d$。70B 是 16KB，7B 是 8KB。16KB 走 IB 的带宽时间 ≈ 0.3µs，几乎可以忽略——但**每步都要传一次，且受固定延迟（NCCL 小消息启动、跨机 RTT（Round Trip Time，网络往返时延））主导**，经验量级每边界 5~10µs（待核实，以你机器实测为准）。PP=4 时每步 3 次传输，约 15~30µs 的固定开销叠在 5.2ms 上不明显；**但注意它是 per-step 固定成本，并发越大、每步出 token 越多，它摊得越薄**——这又回到“PP 是吞吐玩法”的结论。

### 4.4 吞吐账：PP 的收益曲线长什么样

**设问：PP 的吞吐收益和 DP 比，差在哪？** 设并发 $C$、每 step 每 stage 处理一个 microbatch，则：
...
**设问：PP 到底省了多少显存？** 一句话：**每张卡只背自己那个 stage 的层**。
...
### 6.1 为什么行业惯例是 TP 机内、PP 跨机

**设问：组合顺序为什么不能反过来？** 因为两者的通信模式对带宽层级的要求完全不同（模块三 2.1 的结论：机内 NVLink 数百 GB/s，机间 IB 约 50GB/s，差一个数量级）：
...
### 6.2 反例：没有 NVLink 的机器怎么办

**设问：节点内没有 NVLink 时，TP 还成立吗？** 官方建议换 PP。vLLM 官方文档（parallelism_scaling 页）专门写了一条边角规则：**如果节点内 GPU 之间没有 NVLink 互联（比如 L40S 这类机器），官方建议用 PP 而不是 TP**——因为 TP 的每层 AllReduce 在 PCIe 上比 NVLink 慢一个数量级，而 PP 的跨段传输频率低得多，“更低的通信开销、更高的吞吐”。这从反面印证了 6.1 的带宽层级逻辑：**TP 是“吃带宽的娇气方案”，带宽不够时 PP 反而更皮实**。

### 6.3 DeepSeek-V3：训练用 PP，推理为什么不用

最硬的证据来自 DeepSeek-V3 技术报告（§3.2 与 §3.4）：

- **训练**：2048 张 H800 上，**16-way PP + 64-way EP（跨 8 节点）+ ZeRO-1（训练时切分优化器状态的技术）DP**，还专门设计了 DualPipe 双向流水线把 forward/backward 的计算通信重叠——PP 是训练的主力。
- **推理部署**：prefill 阶段 4 节点 32 卡 = **TP4 + SP（序列并行：把序列维度也切开的并行）+ DP8（attention）/ EP32（MoE）**；decode 阶段 40 节点 320 卡 = **TP4 + SP + DP80（attention）/ EP320（MoE）**——**全程没有 PP**。
...
### 7.1 vLLM 的 PP 实现走过哪两代

**设问：vLLM 的 PP 是怎么从无到有的？** 两代实现：
...
**设问：既然 PP 能跨机，TP 跨机为什么不行？** 技术上 vLLM 允许 `--tensor-parallel-size` 跨节点（必须 Ray 拉起，6.2 第 2.4 节算过账：每 step 的 AllReduce 全部走 IB，通信耗时与计算同量级，加速比趋近 1）。**官方文档的推荐始终是“TP = 每节点卡数、PP = 节点数”**——支持跨机 ≠ 推荐跨机。跨节点的活，交给 PP（或 6.4 的 EP）。
