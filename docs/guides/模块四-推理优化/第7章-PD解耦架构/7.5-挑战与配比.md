...
- ❌ **prefill/decode 池配比拍脑袋**：1:1 平均分。为什么错：配比由负载（请求到达率、长度分布）与 SLO 决定，配错导致一边排队一边闲置。正确做法：按本文的配比推导（到达率 × 处理时间 → 池子大小）计算，并留余量。
- ❌ **忽略负载的动态变化**：配比算一次用一年。为什么错：负载（长度分布、QPS）随时间变化，固定配比会在峰值失配。正确做法：配比可调 + 监控池利用率（10.3 的扩缩容联动）。
- ❌ **以为解耦一定优于混批**：无脑拆池。为什么错：解耦的收益在互扰严重时最大；轻负载/短请求为主时拆池的传输与调度开销可能亏本。正确做法：先量化互扰损失（7.1 的方法），收益大于成本再拆。
- ❌ **忽略 KV 传输对 decode 池显存的冲击**：decode 池显存只算权重 + KV。为什么错：接收的 KV 也要占显存（虽然可立即消费），峰值并发时可能爆。正确做法：把传输缓冲与接收峰值纳入 decode 池显存账本。
...
- 能向老板讲清“解耦不提升吞吐，只治尾延迟”（官方文档声明），并指出提吞吐该用什么
...
- [第 10 章 生产部署与运维（10.3 自动扩缩容与 HPA）](/AIInfraGuide/inference/模块四-推理优化/第10章-生产部署与运维/第10章-生产部署与运维)
