...
- ❌ **只测平均延迟就上线混批服务**：Prefill 与 Decode 混在一起只看均值。为什么错：长 prefill 会把同批 Decode 的 P95 拖爆（本文的 2 倍级互扰数据），均值看不出来。正确做法：混批场景必须测 P95/P99 尾延迟（9.1 的指标）。
- ❌ **把 Chunked Prefill 当成互扰的终极解药**：切块了就不管了。为什么错：切块只是缓解，长 prefill 依然占用 GPU，互扰随负载加重回归。正确做法：轻负载切块够用，重负载/严格 SLO 上 PD 解耦（本文的决策条件）。
- ❌ **忽略 goodput 只看 QPS**：压测报告“每秒 100 请求”。为什么错：裸 QPS 不反映 SLO 达标率，混批下高 QPS 可能伴随着大面积尾延迟超标。正确做法：用满足 SLO 的有效吞吐（goodput）评估（7.4）。
- ❌ **以为解耦是免费的**：拆两个池子就完事。为什么错：解耦引入 KV 传输（prefill 池 → decode 池）、配比与调度复杂度（7.3/7.5）。正确做法：先量化互扰损失（本文的方法），再决定是否值得拆。
