...
**三件套分工：SLO 定业务、达标率做监控、goodput 做容量——三件套齐了，调度优化的每一刀都有裁判。**
...
**权衡：Goodput 是用“更复杂的调度”换“更稳的用户体验和更省的资源”——有 SLO 的在线服务值得，纯吞吐的离线任务别跟风。**
...
- ❌ **用裸 QPS 当服务质量的指标**：报表里只有“每秒请求数”。为什么错：QPS 高不代表用户满意——大量请求可能超时（P95 超标），raw QPS 是“吞吐的谎言”。正确做法：用 goodput——满足 SLO 的有效吞吐（本文的公式与算例）。
- ❌ **SLO 用平均值定义**：“平均延迟 < 200ms”。为什么错：均值被快请求稀释，长尾违约看不见。正确做法：SLO 用分位数（P95/P99 TTFT/TPOT 达标率）。
- ❌ **把 goodput 与吞吐混为一个数字**：一个指标走天下。为什么错：goodput 与 SLO 强绑定，SLO 越严 goodput 越低；不同业务（聊天 vs 批处理）SLO 不同，goodput 不可直接比较。正确做法：先定 SLO 再算 goodput，跨业务对比时说明 SLO。
- ❌ **调度只优化平均**：优先级调度只看“先来先服务”。为什么错：不同请求的 SLO 与价值不同（在线 vs 批处理），FIFO 会让在线请求在峰值时违约。正确做法：按 SLO 分级的调度（本文的调度策略）。
