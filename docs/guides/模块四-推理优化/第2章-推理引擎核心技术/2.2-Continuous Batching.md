...
> 💡 **回顾：Memory Bound 是什么**
> Memory Bound（访存受限，算术强度 $I$ 低于硬件屋脊点 $I_{ridge}$，时间卡在搬数据而非算数据；Decode 每 Token 都要搬全量权重 $M_w$，$I\approx 1\,\mathrm{FLOP}/\mathrm{Byte}$ 远低于 H100 屋脊，见 [1.1 LLM 推理基础 §1](/AIInfraGuide/inference/模块四-推理优化/第1章-llm推理基础/11-llm推理基础#1-ttft-vs-tpot)）。本文在此前提下算“多 Token 摊薄一次搬运”的账。
...
⚠️ **注意**：Static Batching 下，GPU 利用率经常只有 **30% 左右**（常见工程口径，源自推理引擎官方博客，本站未复现，待核实）——大量算力浪费在给已完成请求的空槽位做无效计算，同时新请求还必须等整批结束才能进来。请求长度差异越大，浪费越严重。
...
> 💡 **回顾：批处理（Batching）是什么**
> 批处理（Batching，把多个请求的计算合并成一次大矩阵运算，摊薄一次权重搬运成本；如一次搬 $M_w$ 服务 $B$ 个 Token，算术强度 $I(B)=\frac{B\cdot \mathrm{FLOP}_{token}}{M_w}$ 随 $B$ 线性上升，见 [§1 为什么要批处理](/AIInfraGuide/inference/模块四-推理优化/第2章-推理引擎核心技术/22-continuous-batching#1-为什么要批处理)）。本节把“按批”调度的粒度细化到“按迭代步”。
...
📌 **关键点**：Continuous Batching 把 GPU 利用率从约 30% 拉到 **80% 以上**（同上，工程口径，待核实），吞吐随之大幅提升。它的收益来源有两块：一是**完成的请求立刻退出**，不再空转；二是**新请求立刻补位**，不必等整批结束，排队延迟也大幅下降。
...
- **吞吐 vs 延迟**：一步里塞进的请求越多，系统吞吐越高，但每步的前向计算耗时也越长，单请求的 TPOT（Time Per Output Token，每输出一个 Token 的延迟，Decode 步间间隔）会略微变差。单步需搬全量权重 $M_w$，算术强度 $I(B)=\frac{B\cdot \mathrm{FLOP}_{token}}{M_w}$；$B=1\to8$ 时 $I$ 近线性↑，吞吐≈$B/t_{step}$ 提升而 $t_{step}$ 仅微增，阈值由 $M_w/\text{带宽}$ 决定。批次大小需要在两者间取平衡。
...
- ❌ **把静态批处理当“批处理”的默认实现**：等一批请求攒满才开始，批内有人完成也不补位。为什么错：长请求拖累整批，短请求空转等批结束，GPU 利用率可能只有 30% 左右。正确做法：Continuous Batching——iteration 级调度，谁完成谁退出、新请求随时补位。
- ❌ **以为 Continuous Batching 是“调度器黑魔法”**：不理解它改变了什么约束。为什么错：动态组批要求每步迭代都重新安排计算，PagedAttention 的分页是它的地基（显存装得下更多请求才能补位）。正确做法：把“装得下”（PagedAttention）与“喂得饱”（CB）两条线一起理解。
- ❌ **只优化吞吐不管尾延迟**：把 batch 拉到最大。为什么错：batch 越大，长尾请求的排队越久，P95 可能爆炸。正确做法：用 SLO 约束（P95 TTFT/TPOT）反推 batch 上限（9.5 的回归门禁）。
- ❌ **新请求插入策略一刀切**：无条件插队。为什么错：插队会打断正在执行的 Decode 迭代，频繁抢占反而降低整体吞吐。正确做法：用预算/优先级调度（vLLM 的 scheduling budget），新请求在预算富余时才拉入。
