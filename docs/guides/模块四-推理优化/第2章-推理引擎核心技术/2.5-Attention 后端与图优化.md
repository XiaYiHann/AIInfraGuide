...
graph LR
    subgraph 无优化["Decode 逐 Kernel 启动"]
        ...
        B --> C["CPU 发射 Kernel2"]
        C --> D["GPU 算(极快)"]
        D --> E["GPU 等指令约 50%空转<br/>5 µs启动×1000次=5 ms vs 算 10 µs"]
    end
```
...
这就要求 Attention 后端**原生支持混合批次（mixed prefill/decode batch）**：在一次 Kernel 调用里，同时正确处理批中既有 Prefill 又有 Decode 的请求。
> Shape：Prefill `[B_p,S_p,d]→[B_p,S_p,d]`（$S_p$ 可至 4K，切块算），Decode `[B_d,1,d]→[B_d,1,d]`（HBM 访单行 KV），混合批 `[B_p+B_d,max(S_p,S_{cache}),d]` 统一 Kernel 加掩码完成；驻留：HBM 存 KV，SRAM 内算 $QK^T$。FlashAttention 3 等现代后端正是为此设计的——它能接受变长的、混合阶段的输入，用统一的 Kernel 高效算完，而不需要把 Prefill 和 Decode 拆成两次 Kernel 调用。
...
| 状态 | 事件 | 动作 | 新状态 |
|---|---|---|---|
| 未捕获 | 首次遇到该 Batch | 录制（capture）该尺寸图 | 已捕获 |
| 已捕获 | 同尺寸请求到达 | 一次 replay 重放 | 执行中 |
| 执行中 | 尺寸未命中 | 回退逐 Kernel 启动 | 未捕获路径 |
> 📌 关键点解读：状态机让‘录制一次、重放多次、未命中回退’可推演；图中‘一次 replay’对应正文‘成百上千次启动压成一次’。
...
- ❌ **对变长输入硬套 CUDA Graph**：把 dynamic shape 的请求塞进捕获好的图。为什么错：CUDA Graph 要求形状固定，变长输入要么反复重新捕获（开销巨大）要么报错。正确做法：只为预设的 batch/seq 组合捕获图（vLLM 的 padding 策略），或用 V1 引擎的动态图路径。
- ❌ **以为“图优化”是引擎自动完成的**：不配置就期待 Graph 收益。为什么错：CUDA Graph 捕获需要显式开启与预热（`--enforce-eager` 关闭即默认开启，但要为常见形状预热）。正确做法：用 `vllm serve --help` 确认当前版本的图配置，压测对比 eager 与 graph 的耗时。
- ❌ **忽略 FlashAttention 与图优化的相互作用**：分别调优互不关心。为什么错：注意力后端的选择改变 kernel 粒度与显存占用，影响 Graph 的形状集合与捕获效率。正确做法：先定注意力后端（FA2/FA3/FlashInfer），再调图配置，最后统一 benchmark。
- ❌ **把 kernel 启动开销当不存在**：小 batch 下反复启动 kernel。为什么错：每次启动有固定 CPU 开销（微秒级），小 batch 时启动占比大。正确做法：小 batch 用图/融合 kernel 摊薄启动（本文的 5μs × 1000 次算例）。
