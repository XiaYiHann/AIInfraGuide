...
上一章的 KV Cache 显存账本留下了一个尖锐的问题：传统的连续显存分配方式，会因为“预留即浪费”产生大量碎片，实际利用率可能只有两三成（PagedAttention 论文实测：现有系统仅 20.4%-38.2% 的 KV 显存真正存放 token 状态，见 3.8 节的引用）。PagedAttention 就是 vLLM 用来解决这个问题的看家本领——它把操作系统管理内存的经典智慧搬到了 GPU 显存上。这一节我们把它彻底讲透：它借鉴了什么思想、Block Table 怎么工作、为什么能消灭碎片，以及它顺带解锁的前缀共享能力。
...
- ❌ **把“预留即浪费”只当成实现细节**：以为连续显存分配只是“慢一点”。为什么错：预留 + 碎片让 KV Cache 利用率可能只有两三成（论文实测 20.4%-38.2%），batch 起不来，吞吐直接受限。正确做法：理解 PagedAttention 的分页思想——逻辑页 → 物理块的映射，按需分配。
- ❌ **只记“分页”结论，不理解 Block Table**：把 PagedAttention 当黑盒。为什么错：不理解 block table 就无法理解前缀共享、无法理解 kernel 为什么要处理不连续物理块。正确做法：走一遍“逻辑块 → 块表 → 物理块”的映射（本文的图解）。
- ❌ **以为 PagedAttention 解决所有显存问题**：碎片解决了就不管 KV 总量。为什么错：分页消除的是浪费，不是 KV 本身的体积；长序列 + 大并发照样显存不够。正确做法：分页管理 + KV 量化（4.4）+ 前缀共享（2.3）一起上。
- ❌ **忽略共享（Copy-on-Write）的前提**：无脑共享前缀块。为什么错：beam search/并行采样共享的块在任一序列写入时必须复制，共享管理不当会互相污染。正确做法：按 vLLM 的引用计数与 COW 语义管理。
