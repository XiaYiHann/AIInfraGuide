...
为什么差这么多?Qwen2.5-VL 用**动态分辨率**:先按 28×28 的网格把图缩放到目标 token 数(官方建议区间 256~1280,通过 `min_pixels`/`max_pixels` 控制);Llama 3.2 Vision 则是**固定 tile + 大 ViT-H/14**:每 tile 1601 个 patch token,高分辨率图切成最多 4 个 tile——论文实测一张 384×384 的图 1601 token,一张 720p/1080p 的图直达 6404 token。
