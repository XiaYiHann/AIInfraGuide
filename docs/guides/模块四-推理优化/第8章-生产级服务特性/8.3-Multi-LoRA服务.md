...
100 个业务如果各部署一个 7B 全量模型，权重就是 100 × 14GB = 1.4TB；而 Multi-LoRA 用“1 个底座 + N 个 Adapter（适配器，单业务可插拔的小矩阵 $A/B$，见 [1.1 节](#11-从微调一个业务说起)——该节定义 $W'=W_0+BA$；本文复用该定义算账）”把同样的账压到 14GB + 100 × 0.32GB ≈ **46GB**——**显存省 30 倍以上**，并且能在同一个服务进程里按请求热切换业务。这一节先算清 LoRA 的显存账本，再讲 vLLM 的 Multi-LoRA 参数、批内多 Adapter 的共存与驱逐机制、动态拼接的性能代价，最后给出一套“什么时候该用 Multi-LoRA”的决策规则。
...
以 Llama 类 7B 为模板（32 层、hidden=4096（隐藏维度，每 token 向量长度）、FFN（Feed-Forward Network，前馈网络，Transformer 每层两个大矩阵的非注意力部分，参数占比约 2/3，intermediate=11008 为中间层宽度）），LoRA 打在全部 7 个线性层(q/k/v/o/gate/up/down)上，$r = 64$（秩，低秩中间维度，$r\ll d$）：
...
当 Adapter 数量到几千个时，连 CPU 内存都要精打细算。S-LoRA 提出**统一分页（Unified Paging）**：把不同 rank 的 Adapter 权重和不同长度的 KV Cache 放进同一个显存/内存池，按页管理、按需换入。再配合“异构批处理”的 CUDA kernel（把不同 rank 的 Adapter 混在一个 batch 里算），单卡就能服务**数千个**并发 Adapter。吞吐相对 HF PEFT（HuggingFace 的微调库）和当时的朴素 vLLM 实现最高提升 **4 倍**（论文实测口径）。vLLM 的 Multi-LoRA 沿用了同一套思路：预分配槽位 + 按需换入 + 专用 kernel（下一节展开）。
...
vLLM 实际用 **Punica 风格的分段 kernel**（`lora_shrink`/`lora_expand`）把同 batch 里多个 Adapter 的增量计算合并成一次批式 GEMM。做法：同一 Adapter 的请求聚成一段（segment），不同段各算各的低秩乘，再按位置写回。底层推理的“共享底座”和训练时的“共享底座”是同一件事，这正是 1.3 说的机会。
...
**规则：`max-loras` 取“活跃业务数 + 余量”,不是“Adapter 总数”。**

### 4.3 与 Continuous Batching 的交互：rank 补齐的浪费

一个 step 里不同请求的 Adapter 可能 rank 不同（r=16 和 r=64 混批）。kernel 按**批内最大 rank** 预分配计算缓冲区，低 rank 的段会被 padding（补零凑齐长度）——**这部分是纯浪费的算力和带宽**（第 5 节会看到实测量级）。加上每个 step 都要多一轮 LoRA 增量计算，Multi-LoRA 的混批吞吐通常低于纯底座，这是“省显存”换来的成本，不是配置错了。
...
**牺牲什么、换取什么：融合牺牲了“多租户共享”换取“零开销”;动态拼接牺牲了“单请求的少量算力”换取“一套权重服务 N 个业务”。**
