...
上一节用 Roofline 解释了 Decode 为什么被显存带宽卡住：每生成一个 Token，都可能要把模型权重重新搬过一遍。现在再往前问一步：**如果预算固定，新增 FLOPs（浮点运算次数，衡量计算量/算力的单位）应该用来把模型做大，还是用来让模型在推理时多搜索几次？** Snell 等人用 MATH（高中数学竞赛级数学题集）实验给出一个有条件的答案：按题目难度选择推理策略时，达到相近效果最多可以少用约 **4 倍**测试时计算；在 FLOPs 对齐的特定设置下，小模型的推理计算甚至能超过约 **14 倍参数量**的大模型，但最难题仍然存在知识和能力上限。本文不把“想得更久”当成万能按钮。真正要学会的是一笔账：预训练 FLOPs 主要买参数与知识，推理 FLOPs 主要买候选、验证、搜索和修订；两笔账的汇率由模型参数量、预训练 Token 数、推理 Token 数和题目难度共同决定。
...
💡 **回顾：Prefill、Decode、KV Cache 与 Memory Bound 是什么**

自回归生成把一次请求拆成两段：**Prefill** 先并行算出第一个 Token 需要的注意力（重计算、可并行），**Decode** 再一个 Token 一个 Token 顺序生成。为了不在每一步重算历史，模型把已算过的键（Key）和值（Value）缓存下来，这就是 **KV Cache（键值缓存）**。Decode 慢是因为受 **Memory Bound（显存带宽限制）** 拖累——每步都要把权重或缓存搬进计算单元，速度被带宽而非算力卡住；上一节用 **Roofline**（把程序性能画成“受算力上限还是带宽上限哪条线先卡住”的模型）解释的正是这条限制。本文默认你已经读过前情，详见 [1.1 LLM 推理基础](/AIInfraGuide/inference/模块四-推理优化/第1章-llm推理基础/11-llm推理基础)。
...
💡 **回顾与分工：Chinchilla（训练算力分配）与本文（测试时算力分配）是正交的两层账**

Chinchilla 问的是“训练算力固定时，模型参数 $N$ 和训练 Token $D$ 各该分多少”，核心规则是把参数量和数据量按相近比例一起放大（约 20 tokens/parameter）；本文问的是“训练算力与推理算力之间怎么分”。两笔账正交、可以组合：先用 Chinchilla 决定训多大模型，再用本文按题难度决定推理时要不要多搜索。详见 [Chinchilla 精读](/AIInfraGuide/papers/chinchilla-notes)（原文为固定训练算力下 $N$/$D$ 的最优分配）；本文只在“训练 vs 推理”这一层展开，两篇互不替代。
...
论文的训练数据构造是：先并行采样 64 个回答，再从中挑选错误答案和相关的正确答案，构造最多包含 4 个错误答案上下文的 SFT（Supervised Fine-Tuning，监督微调——直接用“输入-正确输出”样本微调模型）样本。推理时，revision model 读取最近的修订结果，生成下一版答案；如果链太长，只保留最近 4 个答案作为上下文。
