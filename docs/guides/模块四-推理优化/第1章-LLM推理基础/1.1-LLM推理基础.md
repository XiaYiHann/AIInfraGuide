...
用数字感受一下这个悬殊。Decode 每步的核心是“权重矩阵 × 单 Token 向量”。对一个权重量为 $W$（字节数）的模型，FP16 下每步：搬运约 $W$ 字节权重，做约 $W$ 次乘加（即约 $2 \times W/2 = W$ FLOP，因 FP16 权重每参数 2 字节、每参数 2 FLOP，两者相消）。于是单请求 Decode 的算术强度约为：
...
- ❌ **用平均延迟做唯一性能指标**：报告“平均 50ms”。为什么错：平均延迟是最容易骗人的指标——长尾请求（P99）超时用户照样卡顿，均值却被稀释得很好看。正确做法：TTFT/TPOT 看分位数（P95/P99），吞吐用并发压测测（9.1 的六件套）。
- ❌ **把 Prefill 和 Decode 混在一个“推理延迟”里**：只报端到端延迟。为什么错：两阶段瓶颈完全不同（Prefill 计算密集、Decode 访存密集），混在一起无法定位优化方向。正确做法：分开测 TTFT（首 token）与 TPOT（后续 token）。
- ❌ **忽略 KV Cache 的碎片化**：以为显存“装得下模型”就万事大吉。为什么错：KV Cache 预留即浪费，连续分配会产生大量碎片，实际利用率可能只有两三成（论文实测 20.4%-38.2%）。正确做法：用 PagedAttention 这类分页管理（第 2 章），并做 KV 显存账本。
- ❌ **以为长上下文是免费的**：把 max-model-len 调大就完事。为什么错：KV Cache 随长度线性增长，序列越长并发越小，吞吐与延迟互相挤压。正确做法：按 3.8 的 KV 账本算清成本，再定 max-model-len。
