...
> **最小走查（2 卡列切输出投影）：** 设权重按列切，卡1算 $[a x_1;\;c x_1]$、卡2算 $[b x_2;\;d x_2]$，相加才得完整 $[a x_1+b x_2;\;c x_1+d x_2]$，故出口必 AllReduce（各卡部分和相加且人人得完整结果）；Ring 通信量约 $2(N-1)/N\cdot M$，$M$ 为层输出大小。
...
- **GQA / MQA / MLA**（Grouped-Query／Multi-Query／Multi-head Latent Attention，多 Query 共享少 KV——GQA 按组共享、MQA 全共享、MLA 把 KV 压到低维潜向量，$H_{kv}$ 32→8 时显存÷4）：当注意力头数小于 TP 规模时（`num_heads < tensor_parallel_size`），KV cache 会在**每张卡上各复制一份**——头都不够分了，只能靠复制保证每卡有完整的 KV 供其处理的头使用。
...
⚠️ **注意**：`enable_attention_dp` 只切换 Attention 模块；模型其余部分（如 FFN）仍按 `tensor_parallel_size` 切分。换句话说，这是“Attention 内部并行方式”的细粒度旋钮，不是全局 DP 开关。并行分工：**小 batch 缺算力 → Attention 上 TP；大 batch 缺显存带宽 → 上 DP**。
...
- **DSA**：DeepSeek V3.2 引入的**模型原生**稀疏注意力——一个轻量 learned indexer 给所有 KV 条目打分，只保留 `index_topk` 个参与注意力。换模型就没得用。完整机制——Lightning Indexer 打分公式、两阶段训练配方、为什么必须建在 MLA 压缩 KV 之上——见 [3.11 从 O(L²) 到 O(Lk)：DeepSeek 稀疏注意力 DSA 详解](/AIInfraGuide/prerequisites/模块一-前置知识/transformer/311-deepseek稀疏注意力dsa详解)——该篇讲机制与训练取舍，本节只讲 TRT-LLM 引擎开关；
...
- ❌ **把 TRT-LLM 的并行当“黑盒开关”**：`--parallel` 开了就完事。为什么错：TP/PP 的通信拓扑与切分要在模型配置里声明，开错组合（如 TP 跨机）性能反而崩。正确做法：按 6.1 的拓扑规则配置（TP 机内、PP 跨机），用本文的配置示例。
- ❌ **量化与并行同时上但互不验证**：每项单独测都通过，合起来崩。为什么错：量化 + 并行（allreduce 精度、反量化 kernel 的并行实现）存在组合问题。正确做法：组合配置做端到端验证（精度 + 吞吐，本文的组合测试）。
- ❌ **忽略引擎版本的特性差异**：教程里的高级特性（如 FP8 并行）在所用版本不可用。为什么错：TRT-LLM 版本迭代快，特性支持随版本变化。正确做法：以所用版本 release notes 为准（本文的版本对照）。
- ❌ **只测吞吐不测精度**：并行量化后吞吐漂亮就上线。为什么错：并行 + 量化可能引入精度损失（allreduce 顺序、量化误差）。正确做法：吞吐与精度（任务指标）一起验收。
...
- 能讲清 Wide-EP 的 expert 与 slot 解耦，并区分 offline 与 online EPLB 的适用场景；
