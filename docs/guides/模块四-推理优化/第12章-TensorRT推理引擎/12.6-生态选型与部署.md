...
> 💡 **回顾：本篇复用的五大机制** —— KV Cache（历史 K／V 缓存，$M=2LH_{kv}d_hSBb_e$，省重算）／PagedAttention（分页显存，块式消碎片，见 [2.1](/AIInfraGuide/inference/模块四-推理优化/第2章-推理引擎核心技术/21-pagedattention)）／IFB（In-Flight Batching，飞行中批处理，迭代级调度，见 [2.2](/AIInfraGuide/inference/模块四-推理优化/第2章-推理引擎核心技术/22-continuous-batching)）／Prefix Cache（前缀命中跳过 prefill，见 [2.3](/AIInfraGuide/inference/模块四-推理优化/第2章-推理引擎核心技术/23-prefix-cache-与-radixattention)）／PD 解耦（Prefill／Decode 分池，见 [第7章](/AIInfraGuide/inference/模块四-推理优化/第7章-pd解耦架构/72-解耦架构设计)——该章讲双池权衡与 KV 传输，本文只作选型复用）。
...
| 前缀复用 | Prefix Cache | KV Cache 复用（含 MLA（Multi-head Latent Attention，多头潜在注意力，把 KV 压缩到低维潜在向量以省显存）复用） |
...
**② 翻车点：`srun --ntasks` 必须与 `--tp_size` 对齐**。`trtllm-llmapi-launch` 按 task 数初始化 rank（分布式进程编号 $0\ldots N-1$，每卡一 rank），`--ntasks 16` 与 `--tp_size 16`（Tensor Parallel，张量并行，按张量维度切权重）对不上会直接起不来；而 `--ep_size 4` 是专家并行（Expert Parallel，按专家切分），在 TP 组内切专家、**不增加 rank 数**——TP16 × EP4 仍是 16 个 rank，不是 64 个。这是多节点部署最常见的翻车点。
...
**官方命令目录（`docs/source/commands/`，核对官方文档所得）只有三个命令：`trtllm-serve`（服务）、`trtllm-bench`（性能基准）、`trtllm-eval`（精度评估）**——三者的分工恰好对应第 9 章的“六件套”与压测/评估两条线（[9.1](/AIInfraGuide/inference/模块四-推理优化/第9章-性能分析与benchmark/91-推理指标体系)、[9.2](/AIInfraGuide/inference/模块四-推理优化/第9章-性能分析与benchmark/92-压测工具)）。
...
**模块四的主线始终是一条：显存、带宽、算力三个瓶颈之间搬资源**（[11.4](/AIInfraGuide/inference/模块四-推理优化/第11章-推理优化选型与端到端实战/114-模块总结与持续学习)的收官大表）。

> 💡 **回顾：PagedAttention 与 Continuous Batching 的落点**
> PagedAttention（分页显存管理，块式映射把碎片从约 20% 降至 95% 利用率，见 [2.1 PagedAttention](/AIInfraGuide/inference/模块四-推理优化/第2章-推理引擎核心技术/21-pagedattention)）在本章落为 Paged KV Cache；Continuous Batching（连续批处理，迭代级调度，见 [2.2 Continuous Batching](/AIInfraGuide/inference/模块四-推理优化/第2章-推理引擎核心技术/22-continuous-batching)）在本章落为 IFB。下文即在此映射上展开选型。

TRT-LLM 是这条主线的“NVIDIA 极速实现版”：Paged KV Cache 是 PagedAttention 的实现（[2.1](/AIInfraGuide/inference/模块四-推理优化/第2章-推理引擎核心技术/21-pagedattention)），IFB 是 Continuous Batching 的实现（[2.2](/AIInfraGuide/inference/模块四-推理优化/第2章-推理引擎核心技术/22-continuous-batching)），FP8/NVFP4 是量化章的官方落点（[4.5](/AIInfraGuide/inference/模块四-推理优化/第4章-量化/45-fp8与nvfp4)），EAGLE/MTP 是投机解码章的实现（[5.1](/AIInfraGuide/inference/模块四-推理优化/第5章-speculative-decoding/51-核心原理与rejection-sampling)），TP/EP 是分布式章的官方参数（[6.1](/AIInfraGuide/inference/模块四-推理优化/第6章-分布式推理/61-推理并行策略总览)），Disaggregated Serving 是 PD 解耦章的 Beta 形态（[7.2](/AIInfraGuide/inference/模块四-推理优化/第7章-pd解耦架构/72-解耦架构设计)）。
...
选型规则：**GPU 锁死 NVIDIA 且要榨干性能 → TRT-LLM；要中立、要社区、要快 → vLLM；要结构化输出与 Agent 场景 → SGLang 值得一试（对比数字以官方 benchmark 为准）**。别为了“显得高级”在两个引擎间反复横跳——先跑通一个，用第 9 章六件套说话。
...
- ❌ **把 TRT-LLM 当 vLLM 的平替**：什么负载都往 TRT-LLM 搬。为什么错：TRT-LLM 强在极限性能与深度定制，但生态、社区、模型支持面不如 vLLM（本文的生态对比）。正确做法：按选型象限——吞吐/社区选 vLLM，极限性能/定制选 TRT-LLM，别为“高级”付维护成本。
- ❌ **忽略部署形态的差异**：TRT-LLM 的部署栈（KServe/llm-d）与 vLLM 不同。为什么错：生产部署的运维、扩缩容、可观测性配套不同，选型要连部署栈一起评估（10.1 的三条路线）。
- ❌ **把官方 benchmark 数字当自己环境的预期**：官方 22% 提升直接当收益承诺。为什么错：官方数字挑模型挑硬件（12.3 已标注口径）。正确做法：用自己负载实测（9.2），官方数字只当上限参考。
- ❌ **忽略版本迁移成本**：从 vLLM 迁移到 TRT-LLM 只比性能。为什么错：迁移涉及 API、配置、运维栈重写，成本要计入选型。正确做法：用本文的迁移清单评估总成本（性能收益 - 迁移成本）。
...
- 能向同事讲清 TRT-LLM 与 vLLM 的本质差异，并说明 SGLang 对比数据为何要标待核实
