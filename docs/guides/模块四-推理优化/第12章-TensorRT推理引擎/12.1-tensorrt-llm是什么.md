...
- **on NVIDIA GPUs（在 NVIDIA GPU 上）**：这句话把它的边界画死了——**它只为 NVIDIA GPU 优化**，这是它与 vLLM 最根本的定位差异之一（详见 [5. 与 vLLM 的定位差异：Day-0 与生态绑定](#5-与-vllm-的定位差异day-0-与生态绑定)——该节对比硬件绑定与发布节奏；本段只点边界）。
...
| 版本 | 关键变化 |
| --- | --- |
| 1.0 | **PyTorch 架构转正并成为默认体验，LLM API 转正稳定**；从 1.0 起实行弃用政策（详见 [8. 遥测与版本政策](#8-遥测与版本政策可关有纪律)——该节讲弃用提前通知与 3 个月迁移期；本行只标里程碑分工） |
