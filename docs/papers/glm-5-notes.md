...
后训练是**串行三阶段 RL**:Reasoning RL → Agentic RL → General RL,最后再来一轮跨阶段蒸馏。Reasoning RL 的算法骨架是 **GRPO + IcePop 变体**。
...
- **Muon 优化器零冗余通信**:all-gather 只取本 rank 拥有的参数分片,和本地计算重叠;
- **激活 offloading + 细粒度重计算**:warmup 阶段 forward 领先 backward,激活存活期长,就挪到主机内存,backward 前再搬回,按层粒度做,和 P2P 通信、MoE token 路由错峰;
...
2. **多节点推理 + DP-attention**:EP64/DP64 摊到 8 个节点,为突发流量备足分布式 KV 容量;DP-attention 专门避免跨 rank 拷贝 KV(想想站内 [7.3 KV 传输](/AIInfraGuide/inference/模块四-推理优化/第7章-pd解耦架构/73-kv传输与connector) 的账);
